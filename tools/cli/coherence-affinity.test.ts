...
import { computeCoherenceAffinityArtifacts } from "./coherence-affinity.js";
import {
...
/** Three rounds in which Cap and Term fall together twice (a strict-majority coupling). */
const coupledRounds = () =>
  Promise.all([
...
describe("computeCoherenceAffinityArtifacts (spec-v32 — document-free exposure affinity)", () => {
  it("walks N artifacts and matches the in-memory computeCoherenceAffinity byte-for-byte", async () => {
    const cs = await coupledRounds();
    const outcome = await computeCoherenceAffinityArtifacts(
      cs.map((c) => buildPostureCoherenceJson(c, LADDER_A)),
      "json",
    );
    expect(outcome.ok).toBe(true);
    if (!outcome.ok) return;
    const inMemory = buildCoherenceAffinityJson(await computeCoherenceAffinity(cs));
    expect(outcome.output).toBe(inMemory);
    expect(outcome.coupled).toBe(true); // Cap & Term fell together both times either fell
  ...
  it("renders a human-readable summary by default", async () => {
    ...
    expect(outcome.ok).toBe(true);
    if (!outcome.ok) return;
    expect(outcome.output).toContain("Coherence exposure co-fall affinity across 4 rounds");
    expect(outcome.output).toMatch(/tightest pairing/);
    expect(outcome.output).toMatch(/affinity_hash: [0-9a-f]{64}/);
  });

  it("trips the gate when two fronts fall together for a strict majority of the steps either fell", async () => {
    const outcome = await computeCoherenceAffinityArtifacts(
      (await coupledRounds()).map((c) => buildPostureCoherenceJson(c, LADDER_A)),
    );
    expect(outcome.ok).toBe(true);
  ...
  it("clears the gate when fronts never fall together more often than apart", async () => {
  ...
  it("the coupled verdict matches exposureCoupled on the same affinity", async () => {
    const cs = await coupledRounds();
    const outcome = await computeCoherenceAffinityArtifacts(
      cs.map((c) => buildPostureCoherenceJson(c, LADDER_A)),
      "json",
    ...
    expect(outcome.ok).toBe(true);
    if (!outcome.ok) return;
    expect(outcome.coupled).toBe(exposureCoupled(await computeCoherenceAffinity(cs)));
  });
});
