/**
 * Vaulytica headless CLI (spec-v8 §22, Step 143).
 *
 *   tsx tools/cli/run.ts analyze <path|glob|dir> \
 *       [--playbook <id>] [--format json,sarif,html,md,csv] \
 *       [--out <dir>] [--fail-on critical|warning|info]
 *
 * Analyzes a file, folder, or glob; writes the selected formats; and exits
 * non-zero when any finding breaches `--fail-on` (the CI gate). The DKB
...
function parseArgs(argv: string[]): Args {
  // argv: ["analyze", target, ...flags]
  if (argv[0] !== "analyze") throw new Error(`unknown command "${argv[0] ?? ""}" (expected: analyze)`);
  const target = argv[1];
