...
const NDA = makePlaybook({
  ...
  custom_rules: [
    {
      id: "NDA-CI",
      title: "Confidential Information must be defined",
      ...
      citation: { reference: "Acme NDA Policy §1" },
    },
    {
      id: "NDA-NO-RESIDUALS",
      title: "No residuals clause",
      description: "We strike residual-knowledge clauses.",
      severity: "warning",
      assert: { kind: "clause_absent", pattern: "residual" },
    },
    {
      id: "NDA-GOVLAW",
      ...
      severity: "warning",
      assert: { kind: "governing_law_in", allowed: ["Delaware", "New York"] },
    },
    {
      id: "NDA-REFS",
      title: "Cross-references must resolve",
      description: "Dangling internal references are unacceptable.",
      severity: "info",
...
const SAAS = makePlaybook({
  ...
  custom_rules: [
    {
      id: "SAAS-LOL",
      title: "Limitation of Liability clause required",
      description: "A limitation-of-liability clause must be present.",
      severity: "critical",
      assert: { kind: "clause_present", pattern: "limitation of liability" },
      citation: { reference: "Acme Contracting Policy §4" },
    },
    {
      id: "SAAS-CAP",
      title: "Liability cap at least 12x fees",
      description: "We do not accept a cap below 12x fees.",
      severity: "critical",
      assert: {
    ...
    {
      id: "SAAS-NOTICE",
      title: "Termination notice at most 30 days",
      description: "We do not accept a termination notice longer than 30 days.",
      severity: "warning",
      assert: {
        kind: "numeric_threshold",
...
const VENDOR = makePlaybook({
  ...
  custom_rules: [
    {
      id: "VND-PAY",
      title: "Payment terms at least Net 30",
      description: "We do not accept payment terms shorter than Net 30.",
      severity: "warning",
      assert: {
        kind: "numeric_threshold",
        metric: "payment_term_days",
        comparator: "gte",
        value: 30,
      },
    },
    ...
    },
    {
      ...
      severity: "warning",
      assert: { kind: "governing_law_in", allowed: ["Delaware", "New York"] },
      citation: { reference: "Acme Vendor Policy §2" },
    },
    {
      id: "VND-REFS",
      title: "Cross-references must resolve",
...
const EMPLOYMENT = makePlaybook({
  ...
  custom_rules: [
    {
      id: "EMP-GOVLAW",
      title: "Governing law must be CA",
      description: "Employment agreements must be governed by California law.",
      severity: "critical",
      assert: { kind: "governing_law_in", allowed: ["California"] },
      citation: { reference: "Acme Employment Policy §1" },
    },
    {
      id: "EMP-NOTICE",
      title: "Notice period at most 30 days",
      description: "We do not accept a notice period longer than 30 days.",
      severity: "warning",
      assert: {
        kind: "numeric_threshold",
        metric: "notice_period_days",
...
describe("custom-playbook end-to-end harness over the fixture corpus", () => {
  it("enforces a per-use-case custom playbook on every fixture and reports findings", async () => {
    ...
    for (const { fixture, playbook, run } of rows) {
      ...
      govLawFindings += run.findings.filter((f) =>
        f.rule_id.toLowerCase().includes("govlaw"),
      ).length;
      uncitedFindings += run.findings.filter(
        (f) => f.citation_provenance === "uncited (team policy)",
      ).length;
