...
/**
 * Each of these cases runs the whole browser bundle pipeline over four
 * members. A fixed 30s was enough in isolation and not under a loaded
 * full-suite run, where vitest schedules this file alongside every other —
 * one of these timed out at 30s while passing in 11s alone. The budget is
 * generous because it is a correctness test, not a performance one: nothing
 * here is measuring elapsed time.
 */
...
describe("browser bundle + privilege-log CSV → production_qa (add-production-qa-pack)", () => {
  ...
  it(
    "emits a production_qa block reconciling the privilege log against the produced set",
    async () => {
  ...
  it(
    "reconciles a privilege-log CSV embedded inside a .zip bundle",
    async () => {
      const zipBytes = zipSync({
        "ACME-000001.docx": new Uint8Array(
          readFileSync(join(BUNDLE_DIR, "master-services-agreement.docx")),
        ),
        "ACME-000003.docx": new Uint8Array(
          readFileSync(join(BUNDLE_DIR, "statement-of-work.docx")),
        ),
  ...
  it(
    "skips production_qa and surfaces both logs when more than one privilege-log CSV is present",
    async () => {
  ...
  it(
    "omits production_qa and leaves bundle_fingerprint byte-identical without a CSV",
    async () => {
  ...
  it(
    "gives two members that share a basename DISTINCT doc_ids (no silent overwrite)",
    async () => {
