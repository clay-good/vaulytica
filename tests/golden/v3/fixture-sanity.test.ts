...
const EXPECTED_RULE_IDS: Record<string, string[]> = {
  ...
  // affirmatively elects not to include a separately captioned clause
  // allocating, as between Customer and Vendor, the intellectual-
  // property rights in the Outputs of any AI system, instead relying
  // on the MSA's general intellectual-property provisions and
  // Section 6 (Vendor Indemnity for AI-Generated IP Infringement) as
  // the operative mechanism for handling any disputes arising in
  // respect of such Outputs. No `(Output\w*).{0,40}(owns?|own|owned
  // by|title to)` or reverse-direction anchor appears anywhere in
  // the document. Section 2 (no-training-on-Customer-Data) is
  // preserved so ADDENDA-011 still passes; Section 6 (IP indemnity)
  // is preserved so the rest of the AI-addendum baseline still
  // passes where it was passing. ADDENDA-013 (warning) fires — its
  // present_patterns
  // `/(Output\w*).{0,40}(?:owns?|own|owned\s+by|title\s+to)/i`
  // and `/(?:owns?|own|owned\s+by).{0,40}Output\w*/i`
  // are no longer matched anywhere in the document. NIST AI RMF and
  // practitioner posture both expect an explicit ownership allocation
  // of AI Outputs; relying on the MSA's general IP provisions alone
  // does not resolve the foundation-specific ambiguity (vendors often
  // claim derivative-work rights in Outputs by default).
  "ai-addendum-missing-output-ownership-fail.txt": ["ADDENDA-013"],

  // AI Addendum with the preamble extended by a sentence that
  // affirmatively elects not to include a separately captioned
  // schedule identifying the third-party model providers Vendor
  // relies on to deliver the AI features described in this AI
  // Addendum, instead relying on the wholesale incorporation of the
  // MSA's sub-contractor authorization mechanism and the MSA-level
  // approved-vendor list as the operative mechanism for downstream
  // model-provider approvals. No OpenAI / Anthropic / Google Gemini /
  // Google Vertex / Cohere / Mistral / Hugging Face / Azure OpenAI /
  // AWS Bedrock anchor and no "AI sub-processor", "model sub-
  // processor", or "AI vendor" anchor appears anywhere in the
  // document. ADDENDA-015 (warning) fires — its present_patterns
  // `/(OpenAI|Anthropic|Google\s+(?:Gemini|Vertex)|Cohere|Mistral|Hugging\s*Face|Azure\s+OpenAI|AWS\s+Bedrock)/i`
  // and `/(AI\s+sub[- ]?processors?|model\s+sub[- ]?processors?|AI\s+vendors?)/i`
  // are no longer matched anywhere in the document. GDPR Art. 28(2)
  // requires controller authorization (and disclosure) of every
  // sub-processor in the chain; when an AI feature is powered by a
  // third-party foundation model, the model provider itself is a
  // sub-processor whose identity must be discoverable from the four
  // corners of the AI Addendum, not buried in a separately maintained
  // MSA-level approved-vendor list.
  "ai-addendum-missing-subprocessor-disclosure-fail.txt": ["ADDENDA-015"],

  // AI Addendum with the preamble extended by a sentence that
  // affirmatively elects not to include a separately captioned clause
  // obligating Vendor to remove, on expiry of the MSA, any datasets
  // derived from Customer Data that were used to adjust the
  // parameters of a foundation model, instead relying on Section 2
  // (which prohibits any use of Customer Data to adjust the
  // parameters of any model) and the MSA's general post-termination
  // handling provisions as the operative mechanism for handling any
  // such residual artifacts. No "delete fine-tuning data",
  // "delete training data", "fine-tuning datasets … delete/destroy",
  // or "model weights … delete/destroy" anchor appears anywhere in
  // the document. ADDENDA-016 (warning) fires — its present_pattern
  // `/(delete\s+(?:fine[- ]tun\w+|training)\s+data|fine[- ]tun\w+\s+(?:data|datasets?)[^.]{0,80}(?:delet|destroy)|model\s+weights?[^.]{0,80}(?:delet|destroy))/i`
  // is no longer matched anywhere in the document. GDPR Art. 17
  // (right to erasure) reaches Customer Data that has been
  // assimilated into model parameters or fine-tuning datasets;
  // relying on a prohibition against training (Section 2) plus a
  // generic MSA wind-down clause does not give Customer a direct
  // contractual hook to compel removal of any residual artifacts
