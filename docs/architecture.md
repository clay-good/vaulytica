...
flowchart TB
    ...
    subgraph browser["The user's browser (this is the entire product)"]
        ...
        engine[Rule Engine<br/>121 launch · 1,825 total rules<br/>+ 22 cross-document checks]
...
- **A presentation form is not a different word.** A ligature (`ﬁ`), fullwidth ASCII (`Ｓｅｃｔｉｏｎ`), a numero sign, a roman-numeral form, and the HYPHEN a PDF emits (U+2010, and Word's non-breaking U+2011) all render as ordinary characters and match none of them. `normalize.ts` folds each back before any rule runs. Every one was found the same way — rewrite the corpus in the form a real file carries and diff the findings ([`format-invariance.test.ts`](../tests/integration/format-invariance.test.ts)) — and every one moved a finding on a large fraction of the corpus, because the fixtures are hand-typed on a keyboard and no example-based test could see it.
- **A word broken across a line is one word.** A justified column marks the break with a hyphen — "confiden-" then "tial" — using the same character, in the same position, as the hyphen a drafter wrote in "month-to-month". [`hyphenation.ts`](../src/ingest/hyphenation.ts) tells them apart on evidence from the document itself: the joined form is used only when the document writes that word as one word somewhere else, and the hyphen is kept otherwise. The default is the conservative one, so a document's existing result can only change on positive evidence. What the conservatism still costs is a committed list in `format-invariance.test.ts` that may only shrink.
...
- **What the file carries that its text does not say.** A DOCX redline reaches the engine as the all-changes-accepted text; hidden `w:vanish` text reaches it although no reader sees it; DOCX comments and PDF annotations never reach it at all. None of that changes _what_ is analyzed — that is a decision for the reader — so the ingest declares it in `IngestResult.warnings` ([`docx-notices.ts`](../src/ingest/docx-notices.ts), `pdf.ts`), and every surface renders it — the tab, the CLI's stderr, the JSON report, the HTML report's cover, an "About This Input" section in the DOCX report, placed ahead of the findings because it qualifies the text every one of them was drawn from, and note-level `VAULYTICA-INPUT-NOTICE` results in the SARIF export — carried as results rather than invocation notifications because CI and code scanning annotate on results.
- **A document the rules cannot read.** Every rule, extractor and playbook in this repo is written against English legal prose, so a Spanish or German contract does not fail — it matches almost nothing and returns a short findings list indistinguishable from a clean one (a measured 3 findings against the English original's 23). [`language.ts`](../src/ingest/language.ts) screens each document by function-word density over a bounded 2,000-word sample, sets `IngestResult.language`, and leads the warnings with a notice that a short findings list is not a clean result. The English floor sits well below the weakest of the 318 English specimens, all of which are pinned as English so the constant cannot drift up into accusing real documents.
...
- **Version comparison** (`compare.ts`, `compare-docx.ts`) — the finding delta of two runs _plus_ the **clause-level redline** (`clause-diff.ts`): a paragraph-level LCS text diff with an inline word-level redline of every rewritten clause, all outside the comparison `result_hash`.
...
`analyze` over two or more inputs also reads them **as a bundle** when asked: `--consistency` runs the cross-document engine (the CC-\* / CROSS-\* rules the browser has run on every multi-document drop since v3), `--emit-consistency` writes the whole `ConsistencyRun`, and `--fail-on-consistency` gates on it — a flag of its own, so turning cross-document checks on never changes the exit code of a job that was already passing. It is asserted rather than inferred because **a directory is not a bundle**: "these two documents name different governing law" is a conflict only between documents from the same deal. With `--format sarif` or `html` the conflicts ride into code scanning and the standalone report, each attached to the document its first excerpt names.

Every report artifact the tab can download, the CLI can write: the fix list, the closing checklist, the critical-dates register and its `.ics`, the obligations ledger, the deadlines calendar, the negotiation posture and its standalone sheet, and the defined-terms CSV — one `--format` value each, gated on the flag that computes its surface. A reach test (`export-reach.test.ts`) holds that correspondence, because nine of those builders shipped, tested, and callable only from the browser until 9.532.0.
