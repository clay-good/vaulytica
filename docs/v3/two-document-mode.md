...
`--format html` renders the same appendix as the DOCX's in the standalone HTML
report. With `--format sarif` the conflicts also reach **code scanning**: each finding
