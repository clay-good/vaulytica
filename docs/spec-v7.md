...
> **Status:** **substantially implemented (version 7.0.0).** Thrust A depth Steps **103–108** (extraction recall: dates, amounts, parties, obligations, definitions, crossrefs/jurisdictions), **110** (three new cross-document families → 13 CROSS-\* total), **113** (mixed-text-layer OCR trigger + per-word confidence), and **114** (report provenance + portfolio executive summary + `.ics` verify-manually events) are done; Thrust B proof Steps **115–122**, **120** (Node↔browser parity), and **125** (responsiveness-as-a-test) are done. **Deferred (principled):** Step **109** (classifier live-routing — held behind the v5 corpus per Open Q #2); Step **111** (jurisdiction-overlay 50-state expansion + non-solicitation family — the state-law enforceability data is attorney-gated under the v5 honesty contract; fabricating it is the dishonesty v5 forbids); Step **112** (rule-catalog depth — each new always-on rule re-baselines every single-document golden's `execution_log` and needs a citable DKB source). **Steps 123–124 (Stryker mutation testing) are now DONE** — scoped to the extractors, scheduled/on-demand, measured baseline 55.65% with a regression-only break floor; and the Step 118/119 property/metamorphic follow-ups (crossref resolve/flag, position-only + order-defect relations) landed. The remaining deferrals are **109/111/112** (all v5-/attorney-gated). See the build-plan table for per-step status.
>
> **Earlier increments (2026-06-05):** Steps 115–116 (Part VIII — code-coverage measurement + regression-only gate) landed: `@vitest/coverage-v8` + `npm run coverage`, a first measured baseline over the shipped `src/` bundle (statements 85.5% · branches 72.4% · functions 87.1% · lines 87.5%), regression-only floors a couple points under each, and the gate wired into `.github/workflows/ci.yml`. **Step 122 (Part XIV — report-structure validation) landed 2026-06-05:** the report DOCX is unzipped and its OOXML asserted (cover proof fields, severity-grouped findings, resolved citations + bibliography, verbatim posture block) and the JSON report's envelope shape is pinned — so the product's actual output, not just its ZIP validity, is proven. **Step 118 (Part X — property-based testing) landed 2026-06-05:** `fast-check` (fixed seed → reproducible) over the foundational layers — normalizer idempotence / exact-offset / drop-empties invariants, plus amount and ISO-date round-trips. **Step 119 (Part XI — metamorphic invariants) landed 2026-06-05, and caught a real bug:** the collapsible-whitespace-noise relation (a document and its whitespace-noised copy must produce the same `result_hash`) failed because `normalize` collapsed whitespace in run text but **not in headings**, so heading whitespace leaked into the offset stream and the hash; fixed in `src/ingest/normalize.ts` with zero golden churn (real headings are clean). **Steps 121 + 117 landed 2026-06-05:** Step 121 (Part XIII — schema fuzz + round-trip) adds round-trip identity + systematic mutation-of-valid rejection across all eight DKB schemas; Step 117 (Part IX — per-rule completeness) is a measure-first regression gate over the always-on launch set (every launch rule runs; 63 fire / 111 go silent / 62 both, of 112, in the golden corpus — floors held a few under). The rest is unimplemented.
...
| # | Step | Output | Tier |
|---|------|--------|------|
| 103 | Dates recall ✅ | Fiscal periods (`fiscal-period` type), broadened citable anchor-alias set + "Date Hereof", disjunctive/range deadlines (`offset_days_max`), documented `TWO_DIGIT_YEAR_PIVOT`; fixtures. **Done 2026-06-05.** | Depth |
| 104 | Amounts recall ✅ | Range amounts (`range_max`; "and" range-connector only after "between"), per-unit qualifiers (`per_unit`), deferred `$`-currency override; optional `MoneyReference` fields; fixtures. **Done.** | Depth |
| 105 | Parties recall ✅ | Alias/role-chain `aliases`, `dba` operating names, two-column signature-field capture; fixtures. **Done.** | Depth |
| 106 | Obligations recall ✅ | Prohibitive/permissive boundary modals (deepened signal — golden re-baseline, OBLI-005 only), nested-trigger decomposition (`nested_triggers`), scope exclusion (`obligor_exclusion`); fixtures. **Done.** | Depth |
| 107 | Definitions recall ✅ | Definition-by-`reference`, `scope`-gated defs, circularity *detection* (`DefinitionMap.circular_terms`; finding-surfacing a gated follow-up to avoid all-golden execution_log churn); fixtures. **Done.** | Depth |
| 108 | Cross-refs + jurisdictions recall ✅ | Sub-reference `sub_ref` capture; governing-law `fallback_jurisdiction` precedence; implied-law inference confirmed absent. All optional fields → zero churn; fixtures. **Done.** | Depth |
| 109 | Classifier live-routing ⏸ | Wire `sub-domain-features.json` into `selectMatchCandidates` behind launch-playbook fallback; golden re-baseline + reviewed diff. **Held behind the v5 corpus (Open Q #2)** — a routing change must be measured against real annotated documents, not synthetic fixtures. | Flagship |
| 110 | Cross-document families ✅ | +3 `CROSS-*` rules — CROSS-TERM-001 (termination-alignment), CROSS-CARVEOUT-001 (limitation-carveout), CROSS-CURRENCY-001 (payment-currency) — each with a bundle fixture; additive (V4_CROSS_RULES 10→13). **Done.** | Depth |
| 111 | Jurisdiction overlays ⏸ | Employment non-compete → 50 states + DC; non-solicitation family. **Deferred (attorney-gated):** per-state enforceability is interpretive legal data; under the v5 honesty contract, fabricating it is the dishonesty v5 forbids. Land when sourced/reviewed. | Depth |
| 112 | Rule-catalog depth ⏸ | Deepen Personnel / Termination / Financial / IP per §15. **Deferred:** each new always-on rule re-baselines every single-document golden's `execution_log`, and each needs a citable DKB source (attorney-gated). | Depth |
| 113 | Ingest robustness ✅ | Per-page text-density OCR trigger (`assessTextLayer`); per-word confidence `[uncertain]` markers (`markLowConfidence`) + ingest warning; deterministic. **Done.** | Depth |
| 114 | Report/export fidelity ✅ | JSON `provenance` (rule-taxonomy version), portfolio `executive_summary`, `.ics` verify-manually events; all outside the run. **Done.** | Deepen |
| 115 | Coverage measurement ✅ | `@vitest/coverage-v8` + `npm run coverage` + committed baseline (statements 85.5% · branches 72.4% · functions 87.1% · lines 87.5% over `src/`). **Done 2026-06-05** (`vitest.config.ts`). | Proof |
| 116 | Coverage gate ✅ | Regression-only floors a couple points under measured (lines 85 · functions 85 · statements 83 · branches 70), wired into `ci.yml`; the cross-OS matrix keeps the plain suite. **Done 2026-06-05.** | Proof |
| 117 | Per-rule completeness meta-test ✅ | Aggregates execution logs across the golden corpus; asserts every always-on launch rule runs, and enforces a regression-only floor on launch-set positive/negative/both coverage (measured 63/111/62 of 112; floors 60/108/59). Full-catalog backfill (family-gated v3/v4) anticipated. **Done 2026-06-05** (`tests/integration/rule-completeness.test.ts`). | Proof |
| 118 | Property-based harness ✅ | `fast-check` (fixed seed, 100 runs) over generated `DocumentTree`s: normalizer idempotence + exact/contiguous/non-overlapping offsets + drop-empties; US-dollar integer round-trip; valid ISO-date round-trip. **Done 2026-06-05** (`tests/integration/property-based.test.ts`, +5 properties). Crossref resolve/flag deferred to a follow-up. | Proof |
| 119 | Metamorphic suite ✅ | Collapsible-whitespace-noise invariance (same `result_hash` + finding set) + whitespace-only-paragraph-insertion invariance, over rule-firing documents; **caught + fixed a heading-whitespace determinism leak in `normalize`** (zero golden churn). **Done 2026-06-05** (`tests/integration/metamorphic.test.ts`, +3 tests; `src/ingest/normalize.ts` fix). Position-only/order-defect relations deferred to a follow-up. | Proof |
| 120 | Node↔browser parity ✅ | Shared fixtures through `src/ui/pipeline.ts` (`runReport`) and `tools/accuracy/pipeline.ts` (`runDocument`); assert byte-identical `EngineRun` (playbook_id + result_hash + findings + execution_log). **Done 2026-06-05** (`tools/accuracy/parity.test.ts`). | Proof |
| 121 | Schema fuzz + round-trip ✅ | Round-trip identity (`parse → serialize → parse`) + wrong-top-level-type rejection + targeted mutation-of-valid rejection (bad enum, missing-required, out-of-range, malformed URL/hash/ISO-datetime) across all eight DKB schemas. Custom-playbook negatives already covered by `custom-playbook.test.ts`. **Done 2026-06-05** (`src/dkb/schema-fuzz.test.ts`, +29 tests). | Proof |
| 122 | Report-structure validation ✅ | Unzip DOCX → assert title + cover proof fields (engine/DKB versions, fingerprint, result hash), findings grouped Critical→Warning→Info, Sources line + Bibliography, verbatim determinism/privacy/non-advice block; JSON envelope shape (run + ingest + well-formed findings). **Done 2026-06-05** (`src/report/docx.test.ts`, +5 tests). | Proof |
| 123 | Mutation measurement ✅ | Stryker (`@stryker-mutator/core` 9.6.1 + vitest-runner) scoped to the date+amount extractors; `npm run mutation` + a scheduled/on-demand workflow (off the per-push path); committed baseline **55.65%** (after a survivor-fix pass that raised it from 51.26%). **Done 2026-06-05** (`stryker.config.json`, `docs/v7/mutation-baseline.md`). | Proof |
| 124 | Mutation gate ✅ | Regression-only `thresholds.break = 48` (a couple points under measured), enforced in `.github/workflows/mutation.yml` only — never the per-push gate. Ratchet up as survivors are killed. **Done 2026-06-05.** | Proof |
| 125 | e2e + a11y expansion ✅ | **Responsiveness-as-a-test** (no horizontal overflow at 320/390/768/1280 px) for landing + empty + complete states, reusing the proven a11y-axe flow. **Done 2026-06-05** (`tests/e2e/responsiveness.spec.ts`). Six-state axe + no-network-across-families remain with the periodic visual audit (Playwright is sandbox-blocked for local verification this session). | Proof |
| 126 | v7 docs + threat-model + version bump ✅ | `docs/v7/` overview + testing-architecture doc; threat-model "v7 — proof surface" note; bump to 7.0.0; reconcile spec statuses. **Done.** | Close |
