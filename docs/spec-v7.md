...
| 123 | Mutation measurement ⏸ | Stryker on `src/engine/` + extractors; separate job; committed mutation-score baseline + top surviving-mutant fixes. **Deferred:** slow, scheduled/on-demand — off the per-push path. | Proof |
| 124 | Mutation gate ⏸ | Regression-only mutation-score floor on covered modules + ratchet; scheduled/on-demand. **Deferred** (depends on 123). | Proof |
