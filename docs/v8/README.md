...
- **Thrust C — Reach** spends the v7 parity proof: SARIF, a single-file HTML report, a headless API + CLI, a playbook diff, a reproducibility verifier, and two export enhancements — the engine the tab runs, now wherever the work happens, byte-for-byte the same.
...
| C · X | **SARIF 2.1.0 export** | rule→`reportingDescriptor`, finding→`result`, finding-id+`result_hash`→`partialFingerprints`, citation→`helpUri` + `properties.citations`; section-primary location, offset in `region`; deterministic canonical JSON. | [`src/report/sarif.ts`](../../src/report/sarif.ts) |
...
| C · XII | **Node API + CLI** | `analyzeText` / `analyzeFile` + a single `vaulytica analyze \| diff \| verify` dispatcher over the parity-proven pipeline (`runIngested`). DKB shipped — no socket. Parity test extended to the API. | [`tools/cli/`](../../tools/cli/) |
...
- **Comparison redline / clause-level text diff** — high-value but requires accurate cross-version span tracking that risks its own correctness bugs; a v8.x candidate.
