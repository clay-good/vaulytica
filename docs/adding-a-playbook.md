...
**A name that sits inside a longer name is not the document's name.** The double credit above is what makes a title decisive, and two families can both earn it on the same title when one of them read only part of it — "assignment and assumption" inside "ASSIGNMENT AND ASSUMPTION OF LEASE", "purchase and sale agreement" inside "RESIDENTIAL PURCHASE AND SALE AGREEMENT", "conflict of interest" inside "CONFLICT OF INTEREST WAIVER". Both scored 0.6 and the alphabet settled it. Two rules now apply, both about containment rather than raw length (a title naming two things — "MASTER SERVICES AGREEMENT AND STATEMENT OF WORK" — has two independent names in it, and neither is inside the other):

- Within a family, nested matches are ONE name. Matching both "subcontract" and "subcontractor agreement" is reading one name at two lengths, not two signals.
- Across families, a keyword another family's matched keyword strictly CONTAINS does not earn the second credit. It is the genus; the containing keyword is the species, and the species is what the document is called.

`tests/integration/title-keyword-reach.test.ts` is the standing measurement: it routes a document titled exactly each declared name in the catalog and ratchets two lists — families no declared name of their own reaches, and names a different family takes. What is left there is genuine collision ("terms of use", "letter of intent", "risk factors", "data processing agreement"), and it is settled in the playbooks by giving one family the name and the other a longer one, not in the matcher.
