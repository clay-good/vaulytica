...
The runner sorts rules by `id` alphabetically (stable, lexicographic). Rules execute serially. The engine records an `ExecutionLog` entry for every rule, capturing `{rule_id, ran: boolean, errored?: true, fired: boolean, finding_id?: string, elapsed_ms: number}` for the audit trail. `ran` is false when the rule was skipped rather than evaluated — a playbook `rule_overrides[id].skip`, or an `applies_to_playbooks` list that excludes the matched playbook. A skipped rule also records `fired: false`, so without `ran` a rule that never executed is indistinguishable from one that executed and stayed silent, and any consumer reading "no finding" as "checked, and clean" overstates how much of the document was screened. `errored` is set when the rule's `check` THREW and the throw was swallowed: the contract is pure, so a throw is treated as silence and the run continues, but the entry that produced would otherwise be identical to the one a rule that ran and found nothing writes — leaving a rule crashing on a real bug to report "screened, and clean" forever. It is omitted rather than `false` on the normal path, so a run in which nothing threw hashes exactly as it did before, and it is inside the hash for the same reason `ran` is: a run in which a rule crashed is a different analysis. Each rule's `check` runs in isolation: it cannot read previous findings, it cannot share state with other rules. Rules that need to share computation can request the same precomputed data from the extractor layer (which is cached for the duration of the run).
...
type ExecutionLogEntry = {
  rule_id: string;
  rule_version: string;
  ran: boolean;                  // false when the rule was skipped, never evaluated
  fired: boolean;
  finding_id?: string;
  elapsed_ms: number;
};
