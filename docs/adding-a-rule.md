...
| A rule wants the TERM OF ART where the document describes the MECHANISM | POL-117 required the phrase "red flag" from an export policy that stops the transaction on a screening hit until Trade Compliance clears it in writing, and self-discloses. The concept was there under every name but the one the rule knew | accept the mechanism as well as the name. The test is whether a correct document could reasonably be written without the term |
| A rule wants a VERB where a heading uses the NOUN | `acknowledge` does not match `ACKNOWLEDGMENT` — the verb has an "e" the noun does not — so EMP-050 reported at `critical` that a handbook whose closing section is headed ACKNOWLEDGMENT had no acknowledgment page | write `acknowledg\w*`. Check every literal verb in a pattern against the noun a HEADING would use |
| The document's OWN NAME is reported as an undefined term | every instrument refers to itself in Title Case — "this Written Consent", "this Guaranty" — and the name is set by the line at the top of the page, not by a definitions section | subtract the title. Read the heading OR the opening line: `sections[0].heading` is the EMPTY STRING for pasted and plain-text documents, which is three separate bugs so far |
| A DEPARTMENT is reported as an undefined term | every policy names the internal function that administers it — "Trade Compliance", "People Operations", "Sales Operations" — and none defines its own org chart | anchor on the function word at the end of the phrase. Nothing calls a defined term "Trade Compliance" |
| A rule demands a STATUTE a compliant document need not cite | CON-021 asked a common-law surety bond for its authority. A school district's bond names no statute because none compels it | ask what the rule's own NAME means in substance. But do not simply drop the pillar: check first whether the remaining pillar reads the TITLE, or you have traded a false accusation for a check no document can fail |
| A document dates itself only at the FOOT, in the formal execution line | "SIGNED AND SEALED this 2nd day of February, 2026" — every bond, deed, will, and affidavit. The first-quarter scan never reaches it and the "Dated this Nth day of" branch wanted the word "Dated" | accept the attesting verbs, spelled in both cases rather than flagged `i` so the month anchor beside them stays case-sensitive |
| A passing REFERENCE to another agreement stands the check down | `amendsParentAgreement` read "the Escrow Amount is released under the Escrow Agreement" as subordination, and one such sentence suppressed seven absence checks on a standalone acquisition agreement | require the document to say that IT is the subordinate one. And check what the over-broad guard was MASKING: tightening it surfaced seven families with no `rule_overrides` at all |
