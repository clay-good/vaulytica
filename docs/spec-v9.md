...
> **Status:** **COMPLETE — all three thrusts shipped (9.0.0).** This spec defines v9 and continues the global step numbering after v8's Step 147, beginning at **Step 148**. **Thrust A (Clean to Send, Steps 148–154)** — the container read-surface ([`src/delivery/`](../src/delivery/)), `HANDOFF-001…005`, the `DeliveryReport` + `delivery_hash`, the adversarial fixtures, and the totality + masking gates — is live and wired into the JSON `delivery` block, the CLI `--delivery`, and the tab's "Clean to send?" section. **Thrust B (Ready to Sign, Steps 155–159)** — `STRUCT-017` signature-block completeness, `STRUCT-018` attachment completeness, `STRUCT-019` recited formalities (all precision-first, internal-consistency only), the readiness consolidation view, and the **Closing Checklist** export ([`src/report/closing-checklist.ts`](../src/report/closing-checklist.ts), Markdown/CSV) — is shipped. **Thrust C (Tracked to Its Dates, Steps 160–164)** — the pure `deriveDate` calendar arithmetic, the `DATE-001…005` derived deadlines, the canonically-sorted **critical-dates register** ([`src/report/critical-dates.ts`](../src/report/critical-dates.ts)) with its own `critical_dates_hash`, the deepened `.ics`/fix-list, and the **no-wall-clock metamorphic gate** — is shipped. The close (Step 165) reconciles statuses and bumps to 9.0.0. The `Status` lines of the prior specs are unchanged by it.
...
| 155 ✅ | Signature-block completeness | `STRUCT-017` — party set ↔ signature-block reconciliation; precision-first (requires a corporate-suffix name, drops defined-term/role phantoms, fires only on a multi-party-labeled block missing a declared party); 0 false positives over the 341-fixture corpus; positive + negative fixtures. | Ready to Sign |
| 156 ✅ | Attachment completeness | `STRUCT-018` — every Exhibit/Schedule/Annex/Appendix/Attachment reference ↔ present heading/title; consolidated reconciliation distinct from `STRUCT-016`'s incorporation lens. | Ready to Sign |
| 157 ✅ | Recited formalities | `STRUCT-019` — recited notary/witness text ↔ presence of a fillable jurat/witness block; internal-consistency only (corollary 2); never asserts legal requirement. | Ready to Sign |
| 158 ✅ | Readiness consolidation | A single ordered "Ready to sign?" view composing `STRUCT-003`/`011`/`013`/`017`/`018`/`019` + `HANDOFF-001`/`002`; render-side, no logic change, no `result_hash` move. | Ready to Sign |
| 159 ✅ | Closing Checklist export | Deterministic itemized checklist in Markdown/CSV ([`src/report/closing-checklist.ts`](../src/report/closing-checklist.ts)) via the v6/v8 export pipeline; JSON `closing_checklist` block; a "Ready to Sign — Closing Checklist" section in the DOCX **and** HTML reports; CLI `--checklist`; structure-tested. | Ready to Sign |
| 160 ✅ | Date-derivation contract | The pure `deriveDate(reference, anchor)` `anchor ± N` function + anchor resolution from the definitions extractor and co-located parentheticals; month-end-clamp / leap-year / business-day-unresolved / unresolved-anchor rules; property tests (validity, monotonicity, month round-trip). New additive `offset_unit`/`offset_count` on `DateReference`. | Tracked to Dates |
| 161 ✅ | Derived deadlines | `DATE-001…005` — auto-renewal notice, cure window, opt-out window, survival end, notice-period; classified from the clause context; responsible party from `obligations.ts`; fixtures + properties. | Tracked to Dates |
| 162 ✅ | Critical-dates register | Canonically-sorted register (computed date · trigger · anchor · responsible · section) with its own `critical_dates_hash`; JSON `critical_dates` block; additive, outside `result_hash`. | Tracked to Dates |
| 163 ✅ | Calendar & fix-list deepening | `buildCriticalDatesIcs` (computed deadlines with a render-only DISPLAY alarm on notice/opt-out/cure rows) + `buildCriticalDatesMarkdown` (grouped register); CLI `--critical-dates`; tab "Your calendar, computed" view; a "Critical Dates" section in the DOCX **and** HTML reports; and `DATE-001…005` as first-class **SARIF** results (note level, section-located). | Tracked to Dates |
| 164 ✅ | No-wall-clock invariant gate | Metamorphic test ([`tests/integration/critical-dates-no-wallclock.test.ts`](../tests/integration/critical-dates-no-wallclock.test.ts)): the same document under two different "today" values yields a byte-identical register, `critical_dates_hash`, `.ics`, and Markdown; the derivation module reads no clock by construction. | Tracked to Dates |
| 165 ✅ | v9 docs + threat-model + version bump | [`docs/v9/README.md`](v9/README.md) overview; threat-model v9 Thrust B/C note; bump to 9.0.0 + `RULE_TAXONOMY_VERSION` 9.0.0; reconcile spec statuses; README posture/test-count + Thrust surface refresh. | Close |
