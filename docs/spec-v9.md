...
| 159 ✅ | Closing Checklist export | Deterministic itemized checklist in Markdown/CSV ([`src/report/closing-checklist.ts`](../src/report/closing-checklist.ts)) via the v6/v8 export pipeline; JSON `closing_checklist` block; CLI `--checklist`; structure-tested. (Standalone DOCX checklist deferred — the main DOCX report still carries the findings.) | Ready to Sign |
...
| 163 ✅ | Calendar & fix-list deepening | `buildCriticalDatesIcs` (computed deadlines with a render-only DISPLAY alarm on notice/opt-out/cure rows) + `buildCriticalDatesMarkdown` (grouped register); CLI `--critical-dates`; tab "Your calendar, computed" view. (HTML/SARIF register-section deferred.) | Tracked to Dates |
