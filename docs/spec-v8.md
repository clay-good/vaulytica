...
> **Status:** **Thrust A (Resilience) implemented; B/C forthcoming.** Steps **128–134** have shipped: the input-boundary guards (byte/paste/depth/OCR caps, decompression-ratio/zip-bomb guard, numeric-magnitude bound, custom-playbook caps), the engine/report scale bound, and the fuzz boundary gate. Step **127**'s adversarial reproductions are embedded in the per-module guard tests rather than a separate `tests/fixtures/adversarial/` tree (a deliberate consolidation — see the build-plan note). **Thrust B (Citations) has started:** Step **135** (inline-everywhere — Markdown link + CSV `authority_url` + the render-side empty-URL/date fix that kills the dangling em-dash) and a partial Step **140** (citation-completeness over Markdown/CSV) have shipped; Steps 136–139 and the SARIF/HTML half of 140 remain, alongside Thrust C (Reach, 141–146). It continues the global step numbering after v7's Step 126, beginning at **Step 127**.
...
| 140 | Citation-completeness gate ◑ | A meta-test (`exports.test.ts`) asserts every cited finding's URL survives into the Markdown + CSV action exports — the executable form of the §14 contract over the formats that exist today. **Extends to SARIF/HTML when Thrust C lands them.** Partial 2026-06-05. | Citations |
