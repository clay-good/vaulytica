...
| 128 | Ingest input guards ✅ | `MAX_DOCUMENT_BYTES`/`MAX_PASTE_CHARS` (typed `InputTooLargeError` before parse); `MAX_SECTION_DEPTH` iterative flatten in `normalize` + iterative `countWords`; `MAX_OCR_PAGES` bound + skipped-pages warning. **Done 2026-06-05** (`src/ingest/limits.ts`). The extractor `walk` helpers (`src/extract/walk.ts`, `forEachParagraph`/`forEachSection`) were made **iterative** in a follow-up (2026-06-08) — they had stayed recursive, so `extractAll` on a directly-passed deep tree threw an uncaught `RangeError`; now stack-safe + fuzz-gated. | Resilience |
...
- **Published npm package / GitHub Action (from §22).** ✅ **Shipped post-8.0.0.** The distribution surface landed once the CLI API stabilized: a `vaulytica` binary (`bin/vaulytica.mjs`) + a publish-ready `package.json` (`bin` + `files` allow-list, `tsx` as a runtime dep so the TS CLI runs with no build step) + a composite GitHub Action (`action.yml`) that runs the engine in any repo's CI (analyze → SARIF code-scanning upload, or a `compare` redline gate). Only the act of publishing (`private: false` + `npm publish`, which needs npm credentials) is genuinely maintainer-gated. Recipes in `docs/ci-integration.md`.
...
- **Comparison redline / clause-level text diff.** ✅ **Done (2026-06-09).** Implemented as `src/report/clause-diff.ts` (`buildClauseDiff`): a deterministic paragraph-level LCS diff of the two documents' own text — rewritten / added / removed clauses — rather than the feared character-level cross-version span tracking, which sidesteps the correctness risk noted here. Bounded (cell ceiling → set-based fallback + `truncated`); rendered into the comparison DOCX ("Document Redline"), the comparison JSON (`clause_diff`), and a UI summary; outside the comparison `result_hash` (zero golden churn).
