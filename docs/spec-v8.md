...
> **Status:** **specified, not implemented.** This is a forward plan; no step below has shipped. It continues the global step numbering after v7's Step 126, beginning at **Step 127**.
...
Emit findings as **SARIF 2.1.0** — the JSON format GitHub Code Scanning, VS Code, and the linter ecosystem consume. A "linter for legal documents" that speaks SARIF can annotate a pull request, populate a code-scanning dashboard, and dedupe findings across runs. The mapping is mechanical and deterministic: each rule → a SARIF `reportingDescriptor` (id, name, `helpUri` = the citation URL); each finding → a `result` (level from severity, message, `locations[].physicalLocation` from the finding's section/offset, `partialFingerprints` from the deterministic `result_hash` so findings dedupe across runs); each `SourceCitation` → the rule's `helpUri` + `result.properties`. Builds on [`src/report/json.ts`](../src/report/json.ts) and [`src/engine/finding.ts`](../src/engine/finding.ts). Deterministic canonical JSON, citable (the citation is the `helpUri`), no posture cost. A `report-structure` test validates the SARIF against its published schema.
...
| # | Step | Output | Tier |
|---|------|--------|------|
| 127 | Adversarial corpus + hardening contract | `tests/fixtures/adversarial/` deterministic builders (nested DOCX, zip bomb, 50-digit amount, ReDoS-bait, 100k-rule playbook, empty/single-glyph/all-whitespace docs); the §5 contract written into [`v8/robustness-and-fuzzing.md`](v8/robustness-and-fuzzing.md); a committed "what currently breaks" baseline. | Resilience |
| 128 | Ingest input guards | `MAX_DOCUMENT_BYTES`/`MAX_PASTE_CHARS` on direct byte/paste entry; `MAX_SECTION_DEPTH` flatten-and-warn in `normalize`/`walk`; `MAX_OCR_PAGES`; deterministic rejection + UI banner. | Resilience |
| 129 | Decompression-ratio guard | Incremental uncompressed-byte budget + `MAX_COMPRESSION_RATIO` in `extractZipEntries`; reject nested archives; zip-slip/`__MACOSX` guards retained; zip-bomb fixture. | Resilience |
| 130 | Numeric & regex bounds | `MAX_AMOUNT_DIGITS` + `NaN`/`Infinity` drop in `amounts.ts`; range-date/range-amount patterns proven bounded against the ReDoS-bait fixture; zero golden churn. | Resilience |
| 131 | Custom-playbook caps | `MAX_PLAYBOOK_JSON_BYTES` (pre-`JSON.parse`), `MAX_CUSTOM_RULES`, per-string length caps in schema; friendly cap-named errors + JSON line:col hint. | Resilience |
| 132 | Engine/report scale bounds | Documented+tested consistency-run bound; `BUNDLE_CROSS_DOC_TOP_N` with honest "N more" footer; serialization-scale assertion. | Resilience |
| 133 | Fuzz harness (measure) | `fast-check` boundary properties (fixed seed) over every public entry point: returns-or-typed-throws, never-uncaught, bounded op-count; committed measured baseline. | Resilience |
| 134 | Fuzz gate (regression-only) | The boundary property becomes a per-commit gate once guards land; the API-boundary analog of v7's metamorphic suite. | Resilience |
| 135 | Inline-everywhere citations | Markdown link + CSV `authority_url`; `.ics` DESCRIPTION source attribution; render-side empty-URL/empty-date fix for cited custom rules; reviewed `exports.test.ts` re-baseline. | Citations |
| 136 | Citation formatter breadth | EU/GDPR, ISO/NIST, secondary-source, and pinpoint-subsection rendering, each tied to a real DKB citation; the coverage matrix in [`v8/citation-standard.md`](v8/citation-standard.md); pinned-string fixtures. | Citations |
| 137 | Freshness & provenance | `source_published_at` populated from real metadata (absent when genuinely unknown — honesty gate), stored on the manifest source record (zero golden churn); reader-facing retrieval-age + "verify currency" signal. | Citations |
| 138 | Wrapping & never-truncate | Long-URL break rendering (DOCX runs + HTML `overflow-wrap`); report-structure assertion that every citation renders in full. | Citations |
| 139 | Citation integrity tool | `tools/citation-check`: per-commit URL well-formedness + scheduled reachability; `accuracy-corpus-guard` extended to assert `src/` never imports it. | Citations |
| 140 | Citation-completeness gate | One meta-test parameterized over all export formats: every finding renders a resolvable (or well-formed URL-less) citation in every format; executable form of the §14 contract. | Citations |
| 141 | SARIF export | SARIF 2.1.0 mapping (rule→reportingDescriptor, finding→result, `result_hash`→partialFingerprints, citation→helpUri); schema-validated. | Reach |
| 142 | Standalone HTML report | Single-file, inlined-CSS, script-free, print-clean report with full inline citations + wrapped URLs; structure-tested. | Reach |
| 143 | Node API + CLI | Stable `runDocument` API + `vaulytica analyze <glob> --playbook --format --fail-on` CLI over the parity-proven pipeline; DKB shipped (no socket); parity test extended to the CLI entry point. | Reach |
| 144 | Playbook diff | `diffPlaybooks(a,b)` → Markdown/JSON structural diff of two custom playbooks (rule selection, severity overrides, custom-rule changes). | Reach |
| 145 | Reproducibility verifier | `verifyReproducibility(report)` re-derives `result_hash` from the provenance block + original doc; reports what diverged. | Reach |
| 146 | Export enhancements | Bundle "everything" archive (per-doc fix-list/CSV/ICS/JSON); clause-evidence coverage surface (optional report section + JSON, outside the run). | Reach |
| 147 | v8 docs + threat-model + version bump | `docs/v8/README.md` overview; threat-model "v8 — hardening surface" note; bump to 8.0.0; reconcile spec statuses; README posture/test-count refresh. | Close |
