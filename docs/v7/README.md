...
| VIII–XIV | **Proof** | Code-coverage gate, per-rule completeness meta-test, property-based testing (`fast-check`), metamorphic invariants, schema fuzz + round-trip, report-structure validation. | [`testing-architecture.md`](testing-architecture.md) |
...
- **Steps 123–124 (mutation testing)** — Stryker is slow; it belongs on a scheduled/on-demand job, off the per-push critical path.
