...
Code coverage says a line _ran_; it does not say a test would _catch a bug_ on that line. A suite can have 90% coverage and still pass when a `>` is silently changed to `>=`. Mutation testing measures the suite's **fault-detection power**: Stryker injects small faults ("mutants") into the source, runs the covering tests, and reports how many mutants the tests _killed_. A surviving mutant is a behavior the tests do not pin — a real weak spot (equivalent mutants aside).
...
and the `^` **anchor** — without which _"Obligations of Section 4"_ is labelled
...
alone over the same 195 mutants. The module joined this set _because_ two of its
...
entire DOB loop (emptiable with nothing failing), the ABA checksum's _rejection_
...
decides _what gets masked_, so it shares `sensitive.ts`'s failure direction. Two
safety paths were pinned on the way in: `maskEmail`'s _"cannot parse this as an
email"_ branch — the safest failure it has, reveal nothing rather than guess at
...
⁴ **Joined 2026-09-08**, measured on its own 502 mutants. The **deadline
arithmetic** — the module behind every date an attorney acts on, so its failure
direction is a wrong date presented with the same confidence as a right one.

It measured **44.02%**, _below_ the aggregate, which under this file's own rule
made it ineligible: widening to a below-mean module drags the aggregate toward
the `break`. So it was raised first, and the raising is the interesting part —
**55 of its 502 mutants had no test executing them at all**, and among them:

- the **backward-counted period** branch under an asserted court profile, whose
  own comment records the audit bug it exists to prevent (`Math.abs()` flipped
  the direction and moved the answer **2N days late**). The regression had a
  fix and a comment and no test.
- the **FRCP 6(d) service-days** sniff — "(Rule 6(d))" was once printed as
  authority on rows that had nothing to do with service, and no test ever
  passed a `service_method` at all.
- both arms of _"the asserted profile could not compute this row"_, which is
  what keeps a register from silently mixing court-rule-correct rows with plain
  arithmetic under one asserted profile.
- every `reason` string `deriveDate` returns **instead of** a date — the
  sentences that carry its "never guess" contract.
- both `return ""` branches of the responsible-party resolver.

🚨 **Two of the new tests were caught vacuous before landing.** One dropped the
anchor _definition_ to make a row unresolved, which meant the branch under test
was never reached (its caller requires an anchor) and it passed with that branch
deleted. And the responsible-party suite asserted only `not.toBe(<the wrong
name>)` — satisfied by _every_ mutant, which is exactly why both of its
`return ""` paths showed as uncovered. Saying **nothing** is the answer there,
and nothing had asserted the empty string.
