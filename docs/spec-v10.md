...
Total work: **10 build steps (166–175).** Thrust A (166–169) is the foundation and is additive — a document run with no positions yields no posture, so the engine `result_hash` never moves. Thrust B (170–172) deepens the surfaces (a standalone sheet, a Markdown/CSV export, and a headless `--posture` CLI mode) — all render-side, zero `result_hash` churn. Thrust C is deferred and specified.
