...
> **Status:** **Thrusts A, B, and C all implemented — A shipped 9.1.0, B shipped 9.2.0, C shipped 9.3.0.** This spec defines v10 on the **v6 bring-your-own-playbook axis** (not the v9 "last look" axis). It continues the global step numbering after v9's Step 165, beginning at **Step 166**. **Thrust A** — the `negotiation_positions` schema extension, the deterministic tier evaluator ([`evaluateNegotiationPosture`](../src/playbooks/custom-interpreter.ts)), the `NegotiationPosture` artifact with its own `posture_hash`, and the report/tab surfaces — is live and wired into the JSON report (`negotiation_posture` block), the DOCX and HTML reports, and the complete-state tab. **Thrust B** — the standalone action-grouped **negotiation sheet** ([`src/report/negotiation-sheet.ts`](../src/report/negotiation-sheet.ts)), the **Markdown/CSV posture export** ([`src/report/exports.ts`](../src/report/exports.ts)), and **headless posture** in the CLI (`--playbook-file <path>` + `--posture`) — is shipped. **Thrust C (Dimension breadth, Steps 173–175)** is shipped: four new `numeric_threshold` metrics (`cure_period_days`, `auto_renewal_notice_days`, `indemnity_cap_amount`, `uptime_sla_percent`) and a `clause_mutual` predicate (one-way vs. mutual indemnity / termination / confidentiality), each **measure-first** — gated behind extractor fixtures proving the extraction is reliable before it is wired. The `Status` lines of the prior specs are unchanged by it.
...
# THRUST B — POSTURE REPORT & EXPORT (shipped, 9.2.0)

Deepens the posture from a report *section* into a standalone, shareable **negotiation sheet** — a one-page view a negotiator works down before a call — a Markdown/CSV export of the ladder, and a headless `--posture` mode in the CLI.

| # | Step | Output | Tier |
|---|------|--------|------|
| 170 ✅ | Negotiation sheet | A standalone, print-clean negotiation sheet ([`src/report/negotiation-sheet.ts`](../src/report/negotiation-sheet.ts)) regrouping the positions by **action** — *escalate* (below floor) · *push here* (at floor) · *verify* (not stated) · *hold* (ideal), in that priority order — so the most urgent fronts are at the top. Escaped, mobile-safe (e2e: vertical-scroll-only 320–1280px, WCAG 2 AA). | Posture report |
| 171 ✅ | Posture export | `buildNegotiationPostureMarkdown` (a dimension · tier · finding · guidance · section table) + `buildNegotiationPostureCsv` (RFC 4180 + formula-injection-guarded — author text is untrusted) via the v6/v8 export pipeline; structure-tested; download buttons in the tab. | Posture report |
| 172 ✅ | Headless posture | The CLI `analyze` command accepts `--playbook-file <path>` (loads + validates a custom playbook; a malformed file is a hard error with the validator's messages) and `--posture` evaluates its `negotiation_positions` against the document, emitting a summary line and the `negotiation_posture` JSON block. (Merging the playbook's custom **rules** into the headless run is a separate follow-up; `--posture` computes the posture only.) | Posture report |

---

# THRUST C — DIMENSION BREADTH (shipped, 9.3.0)

Broadens the set of negotiable dimensions a position can assert on — each measure-first. Four new `numeric_threshold` metrics (cure-period days, auto-renewal-notice days, indemnity-cap amount, uptime-SLA percent), and a structured clause-quality predicate (`clause_mutual` — mutual vs. one-way indemnity / termination / confidentiality) — each gated behind a fixture pass proving the extraction is reliable before it is wired, never guessed. The metrics flow through the **same** `extractMetricValues` path the v6 metrics use; `clause_mutual` reuses the v6 `findClause` locator and adds a bounded, deterministic reciprocity-marker scan (no model). Every new dimension is additive: it widens the authorable DSL without moving any golden or `result_hash`.
...
| 175 ✅ | Mutuality predicates | a `clause_mutual` predicate (one-way vs. mutual indemnity / termination / confidentiality), fixture-gated | Dimension breadth |
...
| 170 ✅ | Negotiation sheet | Standalone, action-grouped print-clean HTML sheet ([`negotiation-sheet.ts`](../src/report/negotiation-sheet.ts)); mobile-safe (e2e). | Posture report |
| 171 ✅ | Posture export | `negotiation-posture` Markdown + CSV via the v6/v8 export pipeline; tab download buttons; structure-tested. | Posture report |
| 172 ✅ | Headless posture | CLI `--playbook-file <path>` ingest + `--posture` mode → summary line + `negotiation_posture` JSON. | Posture report |
...
Total work: **10 build steps (166–175), all shipped.** Thrust A (166–169) is the foundation and is additive — a document run with no positions yields no posture, so the engine `result_hash` never moves. Thrust B (170–172) deepens the surfaces (a standalone sheet, a Markdown/CSV export, and a headless `--posture` CLI mode) — all render-side, zero `result_hash` churn. Thrust C (173–175) widens the authorable dimension set — four numeric metrics and a mutuality predicate — each measure-first (extractor fixtures land before the dimension is wired), and additive: a playbook that does not use the new metrics validates and runs exactly as before.
