...
**An item names what it counted.** Three of the six readiness rules title themselves with a COUNT — `STRUCT-018` "Referenced attachments not present: 3", `STRUCT-017` "Declared parties with no signature line: 2", `STRUCT-013` "Unfilled template placeholders: 4" — and the names are in the finding's `description`. The checklist used to render the title alone, on the premise (written into `labelFor`) that "the finding titles are already one-line and checkable"; on the 327-specimen corpus the checklists produced hold 109 items across 107 documents, and 107 of those items are a bare count. Each item now carries a `detail` — the finding's own one-sentence description, suppressed when it would only restate the label — and every surface renders it: an indented line in the Markdown, a `detail` column in the CSV (`category,rule_id,item,detail,section`), a sub-line in the HTML and tab views, a column in the DOCX table. `src/report/closing-checklist-detail.test.ts` holds the reach list; a new surface that renders a checklist item must join it.
