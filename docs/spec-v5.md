...
> **Status:** partially implemented — the **measurement infrastructure** is built and unit-tested (Steps 67, 69, 71, 83): corpus scaffolding + provenance/redaction tooling, the gold-annotation schema + Cohen's-κ computation, the accuracy harness (`tools/accuracy/`, `npm run accuracy`) with a reproducible scoreboard hash, and the `src/`-never-imports-corpus privacy guard. The **legal-basis ledger scaffolding + the `tier` field on `Rule`/`Finding` (Step 75)** is now in too: the ledger schema + loader (`tools/accuracy/legal-basis.ts`), the honestly-empty machine mirror (`docs/legal-basis/`), and the machine-mirror test that keeps a surfaced `tier` from ever being author-asserted — added additively, so no rule carries a tier yet and `result_hash` is byte-unchanged. The remaining steps gate on **human work that code cannot do** — sourcing real license-clean documents (68, 73, 74), credentialed attorney annotation (70) and legal review (76, 77), and real-user testing (85) — plus the deploy steps (84) that need a live site. See [`docs/v5/methodology.md`](v5/methodology.md), [`docs/v5/annotation-protocol.md`](v5/annotation-protocol.md), [`docs/legal-basis/README.md`](legal-basis/README.md), and [`BUILD_PROGRESS.md`](../BUILD_PROGRESS.md).
...
| 75 | Legal-basis ledger scaffolding + `tier` on Rule/Finding ✅ | Ledger schema + loader (`tools/accuracy/legal-basis.ts`), honestly-empty machine mirror (`docs/legal-basis/ledger.json` + README), machine-mirror test (`tests/integration/legal-basis-ledger.test.ts`: schema validity, no-dup, rule/DKB referential integrity, **inline `Rule.tier` must be ledger-backed**, honest 0-of-N coverage); `tier` field added to `Rule` and `Finding` and threaded through `makeFinding`. Done **additively** — no rule is signed yet, so no finding carries a tier and the golden baseline is byte-unchanged (the deliberate regen happens with the *first* attorney sign-off, Steps 76/77). **Done 2026-06-04.** |
