...
2. **Zip dep selection.** `fflate` is the leading candidate (small, dependency-free, MIT). Confirm against the spec.md §28 closed-source / CDN constraints.
...
4. **State-law overlay scope.** Many v4 families (real estate, employment, lending, trust/estate/family) are sharply state-specific. The DKB needs a per-state overlay node pattern that does not balloon to 50 × N nodes. Proposal: one consolidated node per (sub-domain × state) carrying the deltas from the model law. Discuss before Step 45 lands.
...
7. **The CROSS-PRECEDENCE rule and v3 MSA-027 overlap.** v3 ships MSA-027 (order-of-precedence consistency) as a single-doc check. v4's CROSS-PRECEDENCE catches the same issue across docs. Keep both, or refactor MSA-027 to call into the consistency engine when bundle context is present? Default proposal: keep both, since single-doc users still benefit from the v3 check.
8. **Auto-classifier confidence threshold.** 0.5 sub-domain / 0.5 family is a starting point. May need calibration once fixture corpus exists. Track in BUILD_PROGRESS as a tuning task.
