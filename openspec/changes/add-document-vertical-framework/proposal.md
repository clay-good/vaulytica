...
- **Vertical pack contract, documented and enforced.** A vertical = one classifier family (feature entry in the sub-domain data), one or more playbook/profile JSON files, one rule pack whose every rule declares its gate — `applies_to_playbooks` naming that vertical's ids, or a registered opt-in assertion (flag/toggle, e.g. a deadline-computation profile or `--estate-checks`) for packs that deepen already-shipped playbooks or attach to opt-in machinery — one rule-ID namespace, and one scope-of-review statement ("reviewed for: X; not reviewed for: Y") rendered on every report where the pack ran. A guard test enforces the gate: no rule outside the v1/v2 launch set may lack both a playbook gate and a registered assertion gate.
- **Cross-hash safety pinned.** A property test asserts that adding a vertical pack leaves every existing golden `result_hash` byte-identical when the active playbook is not in the pack's list — and, for assertion-gated rules, whenever the assertion is not made — turning the informal "additive packs don't disturb hashes" invariant into a gate.
