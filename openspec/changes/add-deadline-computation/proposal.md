...
- **DDL-### findings** on top of the register (assertion-gated per the framework: DDL rules declare the `--deadline-profile` assertion as their registered gate — they create no playbook family of their own and run only when a profile is asserted, so unasserted runs stay hash-identical): DDL-001 deadline computed to land on a weekend/holiday *before* rolling (drafting note — the document's own math hits a non-day); DDL-002 conflicting computations for the same obligation (two clauses imply different dates).
