...
- [~] 4. (`--deal-value` done; document auto-extraction DEFERRED — no contract-value extractor yet) Deal-size band resolution: positions carry `size_bands[]` (each `min_value` + a full ladder); `resolvePositionsForDealValue` picks the highest band at/below the resolved value and applies it into a concrete ladder BEFORE eval/hash (so `ladderHash` distinguishes deal sizes; a bandless position is a byte-identical no-op). The applied band is stamped as `size_band` DETAIL on the posture (outside `posture_hash`); an unresolvable/absent value falls back to each position's base default and the report says so ("default (no --deal-value)" / "default (below all bands)"). Verified end-to-end: same 8x doc reads `acceptable` under the ≥$1M band, `ideal` under the default.
