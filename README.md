...
- **Responsiveness- and accessibility-as-a-test** *(Step 125, extended)* — `scrollWidth ≤ clientWidth` at **320 / 390 / 768 / 1280 px** *and* a zero-violation **axe-core WCAG 2 AA** sweep, both turned into CI gates. The live-app specs pin the empty + complete states; companion specs render **every** `DropzoneState` (empty · analyzing · complete · comparison · bundle · error) — in **both** themes — via the real `renderState` + page CSS through `page.setContent` (no server), plus the standalone HTML report, with overflow-*stressing* long contract filenames. That exhaustive pass found and fixed real defects the short-fixture, dark-only live tests never exercised: long unbreakable filenames overflowing `.dropzone-title` / `.dropzone-sub` / `.bundle-rejected-filename` and the HTML report `<h1>` (no `overflow-wrap`); and AA contrast failures — the light-theme `--muted`, a dimmed low-confidence confidence number, the freshness label, the comparison DKB warning — plus a progressbar missing an accessible name.
