...
`1,068 deterministic rules` · `20 cross-document checks` · `5 pre-disclosure checks` · `3 execution-readiness reconciliations` · `5 derived-deadline families` · `16 document sub-domains` · `88 state-law overlays (non-compete · security deposit · usury · will formalities)` · `10 export formats` · `0 servers` · `0 AI` · `4,431 passing tests` · `v9.41.0` · `MIT`
...
flowchart LR
  ...
  F --> G[Run rules<br/>1,068 pure functions]
...
flowchart LR
  ...
  R[(full engine<br/>1,068 rules · 141 playbooks)] --> H
...
A "runs-entirely-in-your-browser" tool ships its whole engine to the client. The trap is obvious: a 1,068-rule engine plus a PDF parser plus a DOCX writer is megabytes of JavaScript, and if it all loads up front the page is slow to paint on the exact phones the product promises to serve. Vaulytica avoids this by splitting the bundle along the **interaction** that needs each piece — nothing parser- or engine-related is on the critical render path. The page paints from ~17 KB gz of self-contained HTML+CSS; everything heavy waits for the file drop.
...
flowchart LR
  ...
  subgraph DROP["On file drop · lazy"]
    P[pipeline · 1,068 rules<br/>268 KB gz]
...
| `pipeline` (1,068 rules + extract/classify/engine) | 268 KB | file drop | you don't need the engine until there's a document to run it on |
...
src/
  ...
  engine/      pure rule runner + 1,068 rules + cross-document consistency
