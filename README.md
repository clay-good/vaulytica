...
`1,808 deterministic rules` · `20 cross-document checks` · `5 pre-disclosure checks` · `3 execution-readiness reconciliations` · `5 derived-deadline families` · `16 document sub-domains` · `88 state-law overlays (non-compete · security deposit · usury · will formalities)` · `10 export formats` · `0 servers` · `0 AI` · `6,700+ passing tests` · `v9.43.0` · `MIT`
...
flowchart LR
  ...
  F --> G[Run rules<br/>1,808 pure functions]
...
On top of that, **v3 (+220 rules)** adds compliance-grade rule sets, **v4 (+771 rules)** adds 16 specialized sub-domains, **v5 (+605 rules)** goes deep on US practice inside those same sub-domains — 105 further American document families, from the purchase order and the franchise disclosure document to the QDRO, the WARN notice, and the stipulated protective order — and **v6 (+92 rules)** turns the same engine on the documents the *practice itself* produces: engagement letters and fee agreements against the ABA Model Rules, discovery requests and responses against the FRCP, and pleadings against Rules 8, 9, 10, 11, and 38. The full **1,808-rule** catalog runs live, family-gated so a plain NDA is not flagged for missing GDPR clauses. The contract every pack honors — how it gates its rules, reserves its namespace, and discloses its scope — is written down in [`verticals`](docs/verticals.md); a document matching no known family is reported as unmatched, never given a confidently-wrong contract verdict.

Those 1,808 are all **single-document** rules. Dropping a folder or `.zip` additionally runs **20 cross-document consistency rules** — defects no single-document scan can see because they live in the *relationship between* documents:
...
| v5 | The US catalog | +605 rules, 105 further US document families inside the same 16 sub-domains | shipped |
...
flowchart LR
  ...
  R[(full engine<br/>1,808 rules · 265 playbooks)] --> H
...
A "runs-entirely-in-your-browser" tool ships its whole engine to the client. The trap is obvious: a 1,808-rule engine plus a PDF parser plus a DOCX writer is megabytes of JavaScript, and if it all loads up front the page is slow to paint on the exact phones the product promises to serve. Vaulytica avoids this by splitting the bundle along the **interaction** that needs each piece — nothing parser- or engine-related is on the critical render path. The page paints from ~17 KB gz of self-contained HTML+CSS; everything heavy waits for the file drop.
...
flowchart LR
  ...
  subgraph DROP["On file drop · lazy"]
    P[pipeline · 1,808 rules<br/>268 KB gz]
...
| `pipeline` (1,808 rules + extract/classify/engine) | 268 KB | file drop | you don't need the engine until there's a document to run it on |
...
npm run lint         # eslint
npm run test         # vitest — 6,700+ tests, ~40s
...
src/
  ...
  engine/      pure rule runner + 1,808 rules + cross-document consistency
