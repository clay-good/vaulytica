...
`1,065 deterministic rules` · `20 cross-document checks` · `5 pre-disclosure checks` · `3 execution-readiness reconciliations` · `5 derived-deadline families` · `16 document sub-domains` · `50 state-law overlays (non-compete · security deposit · usury · will formalities)` · `10 export formats` · `0 servers` · `0 AI` · `4,228 passing tests` · `v9.41.0` · `MIT`
...
# assert the will's state: with a verified formalities overlay (13 verified states) the recital
...
| `--state <xx>` | assert the will's state (implies `--estate-checks`). With a primary-source-verified formalities overlay (13 verified states: PA, LA, CO, ND, VT, CA, TX, NY, FL, IL, MI, NJ, NC) the EST-1xx recital rules speak that state's statute — under PA (20 Pa. C.S. § 2502, no witnesses for an ordinary signed will) absent attestation/witness blocks become info notes; under LA (arts. 1576–1577) a missing notary block escalates to a warning; CO/ND rewording states the notarized-acknowledgment alternative. An unseeded state runs the neutral rules unchanged (honest N/A) and is still stamped into the run |
...
npm run lint         # eslint
npm run test         # vitest — 4,228 tests, ~30s
