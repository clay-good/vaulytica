...
`1,825 deterministic rules` · `20 cross-document checks` · `5 pre-disclosure checks` · `3 execution-readiness reconciliations` · `5 derived-deadline families` · `16 document sub-domains` · `88 state-law overlays (non-compete · security deposit · usury · will formalities)` · `10 export formats` · `0 servers` · `0 AI` · `13,688+ passing tests` · `v9.409.0` · `MIT`
...
flowchart LR
  ...
  F --> G[Run rules<br/>1,825 pure functions]
...
The **always-on launch set** is 121 rules across ten categories that apply to any agreement (the original v1 set of 112, plus v9 Thrust B's three execution-readiness reconciliations and six later dark-pattern additions):
...
| Dark patterns | 15 | unilateral amendment by posting, hidden auto-renewal, browsewrap acceptance, waiver of non-waivable Article 9 protections |
| Personnel | 9 | stay-or-pay/training-repayment clauses, IC misclassification signals, overlong non-solicits |

On top of that, **v3 (+220 rules)** adds compliance-grade rule sets, **v4 (+771 rules)** adds 16 specialized sub-domains, **v5 (+621 rules)** goes deep on US practice inside those same sub-domains — 107 further American document families, from the purchase order and the franchise disclosure document to the QDRO, the WARN notice, and the stipulated protective order — and **v6 (+92 rules)** turns the same engine on the documents the *practice itself* produces: engagement letters and fee agreements against the ABA Model Rules, discovery requests and responses against the FRCP, and pleadings against Rules 8, 9, 10, 11, and 38. The full **1,825-rule** catalog runs live, family-gated so a plain NDA is not flagged for missing GDPR clauses. The contract every pack honors — how it gates its rules, reserves its namespace, and discloses its scope — is written down in [`verticals`](docs/verticals.md); a document matching no known family is reported as unmatched, never given a confidently-wrong contract verdict.

Those 1,825 are all **single-document** rules. Dropping a folder or `.zip` additionally runs **20 cross-document consistency rules** — defects no single-document scan can see because they live in the *relationship between* documents:
...
| v5 | The US catalog | +621 rules, 106 further US document families inside the same 16 sub-domains | shipped |
| v6 | The lawyer's own documents | +92 rules, 15 law-practice families: engagement and fee agreements (ABA Model Rules), discovery instruments (FRCP 26/30/33/34/36), pleadings (FRCP 8/9/10/11/38) | shipped |
...
| v45 | The US Catalog | **105 more American document families, 605 more checks**, inside the same sixteen sub-domains — the satellites an American practice handles all day and reviews least: the purchase order under the MSA, the franchise disclosure document, the QDRO, the WARN notice, the preliminary lien notice, the stipulated protective order. Authored **column-first**: each family's compliance matrix *is* the design and each column ships as exactly one check, with a guard asserting the two never diverge. New shorthand `src/engine/rules/v5/_pack.ts` supplies the boilerplate and leaves the four fields carrying the legal content (citation, patterns, why, fix); severity defaults to `warning` rather than v4's `critical`, because a catalog column is a drafting expectation, not a regulator's enumeration. **The defect this wave taught:** a clause-presence rule fires when *none* of its patterns match, so a rule whose pattern is a word from its own family's title can never fire — a column the matrix shows as reviewed that reports nothing on any document, forever. A probe handing every ungated check the emptiest document its family admits found **27 real instances**, and is now permanent (`title-vacuity.test.ts`). 145 → 250 document types; 1,111 → 1,716 rules | **complete · 9.42.0** (Step 225; [`spec-v45`](docs/spec-v45.md)). |
| v46 | The Lawyer's Own Documents | Every wave through v45 reads a document the lawyer's *client* is a party to. v46 turns the same engine on the documents the **practice itself** produces — the first time Vaulytica reads a document the lawyer signs, and the first time the governing text is a rule the lawyer is personally bound by. Three packs, **15 families, 92 checks**: `ENG` (engagement letters, contingency and flat fee agreements, joint-representation waivers, limited-scope agreements, closing letters — against the ABA Model Rules, concentrating on the *enumerated* requirements like Rule 1.5(c)'s four contingent-fee elements rather than the evaluative ones), `DISC` (requests, responses, privilege logs, Rule 26(f) reports, deposition notices — centred on the 2015 amendments practice has been slow to absorb), and `PLDG` (complaints and answers, form only — the four omissions that cannot be cured later). **Two caveats rendered into the citation rather than a footnote**, because these are the first packs whose governing text mostly does not bind: every Model Rule citation reads "(model text; each state adopts its own version)", and every `DISC`/`PLDG` scope statement lists local rules and standing orders under *not reviewed for*. 250 → 265 document types; 1,716 → 1,808 rules | **complete · 9.42.0** (Step 226; [`spec-v46`](docs/spec-v46.md)). |
| v47 | Legal Currency | Not a feature — the answer to a failure mode the catalog acquired the moment it got large. **Every drift guard in this repo compares the repo against *itself*; an authority going stale is a change in the *world*, and no self-consistency check can see it.** Two were live: the FTC's 2024 "click-to-cancel" amendments (vacated in their entirety July 2025, never took effect) were presented as governing on six surfaces including a rule *named* for them; and the M&A restrictive-covenant playbook still framed itself on the vacated Non-Compete Clause Rule — while three other files in the repo already said correctly that it had been set aside, which is the instructive part: **knowing is not the same as having a mechanism**. Repair pattern, now written down: keep the logic, re-cite to the authority that actually imposes the obligation, **rename if the name carried the dead authority** (the name reaches the findings index, the compliance matrix, and the execution log where the explanation never does), annotate the DKB entry inline, add a registry entry, regenerate both golden families. `tests/integration/vacated-authority.test.ts` makes a repair impossible to undo silently; the per-release re-verification cadence is a stated process commitment, not code | **complete · 9.42.0** (Step 227; [`spec-v47`](docs/spec-v47.md)). |
...
# assert the will's state: the formalities overlay covers all 50 states + DC, so the recital
...
| `compare <base> <revised>` | version-compare two documents + clause redline (Markdown/JSON); `--playbook-file <p> --posture` adds the posture movement; `--fail-on-regression` gates on it; a **cross-family pairing** (the two documents matched different playbooks) is refused unless `--confirm-pairing` is passed, mirroring the tab | `2` when the revision *introduced* a finding at/above `--fail-on`, **or** (with `--fail-on-regression`) when any posture front regressed |
...
| `--state <xx>` | assert the will's state (implies `--estate-checks`). With a primary-source-verified formalities overlay covering ALL 50 states + DC the EST-1xx recital rules speak that state's statute — under PA (20 Pa. C.S. § 2502, no witnesses for an ordinary signed will) absent attestation/witness blocks become info notes; under LA (arts. 1576–1577) a missing notary block escalates to a warning; CO/ND rewording states the notarized-acknowledgment alternative; every witness-expecting seeded state adds EST-107, counting witness signature blocks against the statute's expected witnesses (under CO/ND, silent when a notarial acknowledgment is detected — the statutory alternative — and a both-paths-missing warning otherwise). An unseeded state runs the neutral rules unchanged (honest N/A) and is still stamped into the run |
...
flowchart LR
  ...
  R[(full engine<br/>1,825 rules · 267 playbooks)] --> H
...
A "runs-entirely-in-your-browser" tool ships its whole engine to the client. The trap is obvious: a 1,825-rule engine plus a PDF parser plus a DOCX writer is megabytes of JavaScript, and if it all loads up front the page is slow to paint on the exact phones the product promises to serve. Vaulytica avoids this by splitting the bundle along the **interaction** that needs each piece — nothing parser- or engine-related is on the critical render path. The page paints from ~17 KB gz of self-contained HTML+CSS; everything heavy waits for the file drop.
...
flowchart LR
  ...
  subgraph DROP["On file drop · lazy"]
    P[pipeline · 1,825 rules<br/>268 KB gz]
...
| `pipeline` (1,825 rules + extract/classify/engine) | 268 KB | file drop | you don't need the engine until there's a document to run it on |
...
`npm run verify` **is** the CI gate — the same commands CI runs, in the same order, pinned to `.github/workflows/ci.yml` by [`verify-mirrors-ci.test.ts`](tests/integration/verify-mirrors-ci.test.ts) so the two cannot drift. Run it before pushing; the individual steps below are for iterating.

```
npm run verify       # typecheck + lint + format:check + coverage + build — exactly what CI runs
npm run build        # static site → dist/
npm run typecheck    # tsc --noEmit
npm run lint         # eslint
npm run test         # vitest — 13,688+ tests, ~140s
...
src/
  ...
  engine/      pure rule runner + 1,825 rules + cross-document consistency
