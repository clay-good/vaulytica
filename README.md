...
- **Responsiveness-as-a-test** *(Step 125, extended)* — `scrollWidth ≤ clientWidth` at **320 / 390 / 768 / 1280 px** turns the recurring manual mobile audit into a CI gate. The live-app spec pins the empty + complete states; a companion spec renders **every** `DropzoneState` (empty · analyzing · complete · comparison · bundle · error) via the real `renderState` + page CSS through `page.setContent` (no server), with overflow-*stressing* long contract filenames. That exhaustive pass found and fixed three real overflows — a long underscore-joined filename in `.dropzone-title` / `.dropzone-sub` / `.bundle-rejected-filename` was a single unbreakable token with no `overflow-wrap` — that the short-fixture live test never exercised.
