...
const VENUE =
  /\b(?:venue|forum|exclusive\s+jurisdiction)\s+(?:shall\s+be|is|lies)\s+(?:in\s+)?(?:the\s+(?:state|federal)\s+courts?\s+(?:located\s+in\s+|of\s+|in\s+))?(?:the\s+(?:State|Commonwealth)\s+of\s+)?([A-Z][A-Za-z\s&-]+?)(?=[.,;)]|\s+and|$)/gi;
