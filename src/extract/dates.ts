...
// fell to the RELATIVE pass and lost its ISO value, so STRUCT-002 reported "No
// Effective Date" on a contract that plainly states one.
...
const ANCHOR_ALIASES =
  "Effective|Closing|Commencement|Termination|Expiration|Renewal|Execution|Signing|Start|Term Start|Delivery|Acceptance|Go-Live|Hire|Grant|Vesting|Maturity|Funding|Disbursement|Completion|Onboarding|Anniversary|Separation|Settlement|Distribution|Conversion|Exercise|Issuance|Record";
...
export function extractDates(tree: DocumentTree): DateReference[] {
  ...
  forEachParagraph(tree, (ctx) => {
    ...
    // The "Nth day of Month, Year" absolute date, recorded before RELATIVE so
