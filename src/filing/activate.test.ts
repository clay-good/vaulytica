...
describe("activateFiling", () => {
  it("is a no-op when no court profile is selected", () => {
  ...
  it("appends the FILE rules and stamps the profile for a filing playbook", () => {
    const w = activateFiling(
      { profile: FRAP, brief_kind: "principal" },
      "appellate-brief",
      ingest,
    ...
    expect(w.rules.length).toBe(LAUNCH_RULES.length + FILING_RULES.length);
