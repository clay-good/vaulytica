...
describe("Launch rule registry", () => {
  it("ships exactly 112 rules (80 launch + 32 post-1.0 additions)", () => {
    expect(LAUNCH_RULES.length).toBe(112);
  });
...
describe("Engine + all 112 rules", () => {
  it("runs end-to-end on a minimal context and is deterministic", async () => {
    ...
    expect(a.execution_log.length).toBe(112);
  });
});
