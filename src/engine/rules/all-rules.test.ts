...
describe("Launch rule registry", () => {
  it("ships exactly 117 rules (80 launch + 34 post-1.0 + 3 v9 Thrust-B reconciliation)", () => {
    expect(LAUNCH_RULES.length).toBe(117);
  });
...
describe("Engine + all 117 rules", () => {
  it("runs end-to-end on a minimal context and is deterministic", async () => {
    ...
    expect(a.execution_log.length).toBe(117);
  });
});
