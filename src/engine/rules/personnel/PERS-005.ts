 ...
 */
/**
 * The shapes that MENTION a non-compete without imposing one: a disclaimer
 * ("nothing shall be construed as a covenant not to compete"), a drag-along's
 * protection ("no Stockholder shall be required, as a condition of the
 * drag-along, to agree to any non-competition covenant"), and the incoming-
 * obligations representation every offer letter carries ("you are not subject
 * to any non-competition agreement").
 *
 * Exported because PERS-001 reports the same clause from the other side —
...
export const rule: Rule = {
  ...
  check(ctx: RuleContext): Finding | null {
    ...
    // a non-compete clause present. The drag-along states its
    // condition in a PARENTHETICAL — "No Stockholder shall be required, AS A
    // CONDITION OF THE DRAG-ALONG, to agree to any non-competition covenant" —
    // and the adjacent "required to" could not reach past it.
    //
    ...
    const hit = hits.find(
      (h) =>
        !NON_COMPETE_DISCLAIMED.test(h.text) && !describesCovenantElsewhere(h.text, h.match.index),
    );
    if (!hit) return null;
