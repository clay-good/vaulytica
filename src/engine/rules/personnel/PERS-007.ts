  ...
  check(ctx: RuleContext): Finding | null {
    ...
    forEachParagraph(ctx.tree, (p) => {
      ...
      if (
        ...
        /\bcompany-?supplied\s+(?:equipment|computer|laptop|hardware|tools)|use\s+(?:the\s+)?company['’s]+\s+(?:equipment|computer|laptop|systems)|company\s+(?:shall|will|must)\s+provide\s+(?:all\s+)?(?:equipment|hardware)/i.test(
          p.text,
        )
      ) {
