...
const ENUMERATED_IN_DEFINITION = new RegExp(
  String.raw`\b(?:means|(?:shall|will|must)\s+mean)\b[^.]{0,300}?(?:${DISPUTE_SIBLING}\s*,\s*(?:or\s+)?arbitration\b|\barbitration\s*,\s*(?:or\s+)?${DISPUTE_SIBLING}|\barbitral\s+${DISPUTE_SIBLING})`,
  "i",
);
