...
const URL_BY_CITATION: Record<string, string> = {
  ...
  "CCPA § 1798.130":
    "https://leginfo.legislature.ca.gov/faces/codes_displaySection.xhtml?lawCode=CIV&sectionNum=1798.130",
  ...
  "COPPA 16 C.F.R. § 312.4":
    "https://www.ecfr.gov/current/title-16/chapter-I/subchapter-C/part-312/section-312.4",
...
export const ADDENDA_RULES: Rule[] = [
  ...
  presenceSec({
    ...
    missing_description:
      "No enumeration of specific security controls (access control, encryption, vulnerability management, etc.) was found.",
    explanation:
      "A pure 'industry-standard' commitment is unenforceable. Customers need an enumerated list to verify and audit against.",
    recommendation:
  ...
  presenceSec({
    ...
    missing_description:
      "No cadence (annual / quarterly / etc.) for security reviews / SOC 2 audits / ISO 27001 reviews was found.",
  ...
  presenceSec({
    ...
    missing_description:
      "Neither a customer right-to-audit nor a SOC 2 / ISO 27001 substitution was found.",
    explanation:
      "Customers must be able to verify the vendor's security posture either directly (audit) or via an independent attestation.",
    recommendation:
      "Add: 'Customer may audit Vendor's security controls annually with reasonable notice, or in lieu of an audit Vendor will deliver a current SOC 2 Type II report.'",
    present_patterns: [
      /(right\s+to\s+audit|customer.{0,40}audit|SOC\s*2|ISO\s*27001|ISO\s*\/IEC\s*27001|in\s+lieu\s+of\s+(?:an\s+)?audit)/i,
    ],
    default_severity: "warning",
  }),
  presenceSec({
  ...
  presenceSec({
    ...
    missing_description:
      "No vulnerability-disclosure / coordinated-disclosure / bug-bounty policy reference was found.",
    explanation:
      "A documented disclosure pathway materially reduces the time-to-fix for vulnerabilities reported by researchers.",
    recommendation:
      "Reference a vulnerability-disclosure policy (e.g., 'Vendor maintains a coordinated vulnerability disclosure policy and acknowledges reports within 5 business days.').",
    present_patterns: [
  ...
  presenceSec({
    ...
    missing_description:
      "No reference to a secure-development-lifecycle (SDLC), SAST/DAST, or code-review program was found.",
    explanation:
      "An SDLC program is the practitioner-accepted way to ensure vulnerabilities are caught before code reaches production.",
    recommendation:
  ...
  presenceSec({
    ...
    missing_description:
      "No data-classification / tiering / sensitivity-labeling scheme was found.",
    explanation:
      "Without classification, all Customer Data is implicitly treated at the same (often lower) tier.",
    recommendation:
      "Add: e.g., 'Customer Data containing personal data is classified as Confidential and receives the controls in Schedule X.'",
    present_patterns: [
  ...
  presenceSec({
    ...
    missing_description:
      "No named encryption standard (FIPS 140-3 / AES-256 / TLS 1.2 or 1.3) was found.",
    explanation:
      "Generic 'encryption' is unauditable; named standards are the practitioner-accepted contract anchor.",
    recommendation:
      "Name the standards: e.g., 'AES-256 at rest, TLS 1.2 or 1.3 in transit, FIPS 140-3 validated modules where applicable.'",
  ...
  presenceAi({
    ...
    description:
      "AI Addendum must define the core terms (Generative AI, Foundation Model, Output, Training Data).",
    citation: "NIST AI RMF (consensus practice, not statute)",
    missing_title: "AI definitions missing",
    missing_description:
      "Core AI terms (Generative AI, Foundation Model, Output, Training Data) were not all defined.",
    explanation:
      "Definitions anchor every other AI-addendum obligation; without them, key terms are ambiguous.",
    recommendation:
  ...
  presenceAi({
    ...
    present_patterns: [
      /(AI\s+features?|AI\s+functions?|generative\s+features?)/i,
      /(on\s+by\s+default|opt[- ]in|opt[- ]out|enabled\s+by\s+default)/i,
      /(third[- ]party\s+(?:provider|model|host)|on[- ]prem|on[- ]premises|self[- ]hosted)/i,
    ],
    default_severity: "warning",
  }),
  presenceAi({
    ...
    missing_description:
      "No clause allocates IP ownership of Outputs as between Customer and Vendor.",
    explanation:
      "Output ownership is a frequent dispute and varies materially across vendors; allocation must be explicit.",
    recommendation:
      "Allocate explicitly: e.g., 'As between the parties, Customer owns Outputs to the extent permitted by law; Vendor retains its models and tooling.'",
    present_patterns: [
      /(Output\w*).{0,40}(?:owns?|own|owned\s+by|title\s+to)/i,
      /(?:owns?|own|owned\s+by).{0,40}Output\w*/i,
    ],
    default_severity: "warning",
  }),
  presenceAi({
    ...
    description:
      "AI Addendum should disclaim warranties on AI outputs (hallucination risk) and obligate human review for high-stakes uses.",
    citation: "NIST AI RMF (consensus practice, not statute)",
    missing_title: "AI hallucination disclaimer / human-review obligation missing",
    missing_description:
      "No hallucination-risk acknowledgment or human-review obligation for high-stakes outputs was found.",
    explanation:
      "Practitioner posture is: AI outputs are not fit for legal / medical / financial advice without human review.",
    recommendation:
      "Add: 'Outputs are provided as-is and may contain inaccuracies; Customer is responsible for human review prior to use for legal, medical, or financial decisions.'",
    present_patterns: [
      /(hallucinat\w+|inaccurate\s+output|not\s+fit\s+for|not\s+(?:professional|legal|medical|financial)\s+advice)/i,
      /human\s+(?:review|oversight|in\s+the\s+loop)/i,
    ],
    default_severity: "info",
  }),
  presenceAi({
    ...
    explanation:
      "When AI features are powered by a third-party model provider, GDPR Art. 28(2) requires authorisation and disclosure.",
    recommendation:
      "List AI subprocessors by name (e.g., OpenAI, Anthropic, Google, Cohere) or reference a maintained URL.",
    present_patterns: [
  ...
  presenceAi({
    ...
    missing_description:
      "No obligation to delete fine-tuning / training data on termination was found.",
    explanation:
      "Without deletion, Customer Data can persist in vendor model weights indefinitely.",
    recommendation:
  ...
  presenceEula({
    ...
    explanation:
      "An EULA without an explicit grant defaults to copyright law's narrow defaults; without prohibited uses, abuse vectors are unaddressed.",
    recommendation:
  ...
  presenceEula({
    ...
    description:
      "EULAs distributed in the EU should reference Digital Content Directive 2019/770 minimums (conformity, defects, remedies).",
    citation: "EU Digital Content Directive 2019/770",
    missing_title: "EU consumer-law minimums missing",
    missing_description:
      "No reference to EU consumer-law minimums (Digital Content Directive 2019/770 or CRD 2011/83) was found.",
    explanation: "EU consumer law cannot be contracted around for consumer-facing EULAs.",
    recommendation:
      "Add: 'Nothing in this EULA limits the consumer rights conferred by Directive (EU) 2019/770 on digital content or Directive 2011/83/EU on consumer rights.'",
    present_patterns: [
  ...
  presencePrivacy({
    ...
    description:
      "Privacy policy should contain CCPA § 1798.130 / GDPR Art. 13–14 / COPPA disclosures where applicable.",
    citation: "CCPA § 1798.130",
    missing_title: "Required privacy disclosures missing",
    missing_description:
      "Required privacy-policy disclosures (CCPA categories of personal information / GDPR Art. 13–14 lawful basis / COPPA child-data disclosures) were not all detected.",
    explanation:
      "Privacy-Policy-Lint does not opine on goodness; it verifies the regulator-required disclosures are present so a reviewer can audit alignment.",
    recommendation:
      "Include: categories of personal information collected, sources, purposes, third-party sharing, retention, lawful basis (GDPR), data-subject rights, COPPA child-data provisions where applicable.",
    present_patterns: [
