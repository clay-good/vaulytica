...
const COMPLIANT_SECURITY: [string, ...string[]][] = [
  ...
  ["Access Control", "Vendor enforces multi-factor authentication (MFA) and least-privilege access control for all production systems."],
  ["Encryption", "All Customer Data is encrypted at rest using AES-256 and in transit using TLS 1.2 or TLS 1.3. FIPS 140-3 validated modules are used where applicable."],
  ["Vulnerability Management", "Vendor maintains a vulnerability management program with continuous scanning, patching, and configuration management."],
  ["Audit Rights", "Customer may audit Vendor's security controls annually with reasonable notice; in lieu of an audit Vendor will deliver a current SOC 2 Type II report renewed annually."],
  ["Incident Response", "Vendor shall notify Customer within 48 hours of confirming a security incident affecting Customer Data."],
  ["Vulnerability Disclosure", "Vendor maintains a coordinated vulnerability disclosure (VDP) program and acknowledges reports within 5 business days."],
  ["Secure Development", "Vendor operates a secure-development-lifecycle (SDLC) including SAST, DAST, and peer code review prior to production deployment."],
  ["Data Classification", "Customer Data containing personal data is classified as Confidential and receives the controls in Schedule A. Data classification is mapped to control tiers."],
  ["Penetration Testing", "Vendor commissions an independent penetration test annually and remediates critical findings within 30 days."],
];

const COMPLIANT_AI: [string, ...string[]][] = [
  ["AI Addendum", "This Addendum governs Generative AI features."],
  ["Definitions", "'Generative AI' means a foundation model that produces Output. 'Foundation Model' shall mean a large language model. 'Output' means the data produced by the model. 'Training Data' means data used to train models."],
  ["Permitted and Prohibited Uses", "Vendor will not train its models on Customer Data without an explicit opt-in by Customer. Opt-out is insufficient."],
  ["Transparency", "AI features in the Service are listed in Schedule B and are opt-in by default. The underlying foundation model is hosted by a third-party provider (OpenAI) or on-prem where indicated."],
  ["Intellectual Property", "As between the parties, Customer owns Outputs to the extent permitted by law; Vendor retains its models and tooling."],
  ["Warranties and Disclaimers", "Outputs are provided as-is and may contain inaccuracies (hallucination risk). Customer is responsible for human review prior to use for legal, medical, or financial decisions."],
  ["Subprocessors", "AI subprocessors include OpenAI, Anthropic, and Google Vertex; the current list is maintained at the URL in Schedule C."],
  ["Data Handling", "On termination, Vendor shall delete all fine-tuning data derived from Customer Data within 30 days; trained model weights derived from Customer Data shall be deleted within 90 days."],
];
...
describe("Addenda ruleset — registry contract", () => {
  ...
  it("does not run when no addenda playbook is active", async () => {
    const ctx = buildContext(["Agreement", "Generic services agreement, no addenda playbook active."]);
    const run = await runEngine({ rules: ADDENDA_RULES, ctx, executed_at: "2026-05-13T00:00:00Z", source_file: SRC });
  ...
  it("security rules don't fire on the AI playbook", async () => {
    ...
    const run = await runEngine({ rules: ADDENDA_RULES, ctx, executed_at: "2026-05-13T00:00:00Z", source_file: SRC });
    const securityIds = run.findings.map((f) => f.rule_id).filter((id) => /ADDENDA-00[1-9]/.test(id));
...
describe("Addenda ruleset — determinism", () => {
  it("two runs over the same input produce the same result_hash", async () => {
    const ctx = withPb(buildContext(...COMPLIANT_SECURITY), SEC);
    const a = await runEngine({ rules: ADDENDA_RULES, ctx, executed_at: "2026-05-13T00:00:00Z", source_file: SRC });
    const b = await runEngine({ rules: ADDENDA_RULES, ctx, executed_at: "2026-05-13T00:00:00Z", source_file: SRC });
