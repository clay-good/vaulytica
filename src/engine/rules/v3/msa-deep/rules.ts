...
const CONFIG: RegulatedRuleConfig = {
  ...
  cite_for(citation: string) {
    ...
    if (lower.includes("§ 1668") || lower.includes("section 1668") || lower.includes("california civ")) {
      url = "https://leginfo.legislature.ca.gov/faces/codes_displaySection.xhtml?lawCode=CIV&sectionNum=1668";
    } else if (lower.includes("5-322.1") || lower.includes("ny gen") || lower.includes("new york general obligations")) {
    ...
    return { id: `msa-deep-${citation.replace(/[^A-Za-z0-9]+/g, "-").toLowerCase()}`, source_url: url };
  },
};
...
export const MSA_DEEP_RULES: Rule[] = [
  ...
  presence({
    ...
    explanation: "A vendor-supplied product that infringes a third party's IP exposes the customer to suit. The standard allocation is that the vendor indemnifies for IP claims.",
    recommendation: "Add a third-party IP infringement indemnity (defense, settlement, damages) with the usual customer-cooperation conditions.",
    present_patterns: [/(?:indemnif\w+).{0,200}(?:intellectual\s+property|infring\w+|IP\s+claim)/is],
    default_severity: "warning",
  }),
  presence({
    id: "MSA-002",
    name: "Indemnification procedure (notice / defense / settlement)",
    description: "Indemnification clause must include procedural mechanics (prompt notice, control of defense, settlement consent).",
    citation: "Commercial drafting baseline — indemnification procedure",
    missing_title: "Indemnification procedure unspecified",
    missing_description: "No prompt-notice / control-of-defense / settlement-consent language found alongside the indemnity.",
    explanation: "Without procedural mechanics the indemnity is ambiguous: who controls defense, who consents to settlement, what notice is required.",
    recommendation: "Add: prompt written notice, indemnitor controls defense with reputable counsel, settlement requires indemnitee consent for non-monetary terms.",
    present_patterns: [/(prompt(?:ly)?\s+(?:notify|notice)|control\s+of\s+(?:the\s+)?defense|settlement.{0,40}consent)/i],
    default_severity: "warning",
  }),
  presence({
    id: "MSA-003",
    ...
    explanation: "Confidentiality breaches often produce damages that are difficult to quantify; indemnification anchors the remedy.",
    recommendation: "Add an indemnity for damages arising from breach of confidentiality obligations.",
    present_patterns: [/(?:indemnif\w+).{0,200}(?:confidential\w+|confidentiality\s+(?:breach|obligation))/is],
    default_severity: "info",
  }),
  presence({
    id: "MSA-004",
    name: "Indemnity for breach of data protection / gross negligence / wilful misconduct",
    description: "Indemnification must cover breach of data-protection obligations, gross negligence, or wilful misconduct.",
    ...
    missing_description: "No indemnification covering breach of data protection, gross negligence, or wilful misconduct was found.",
    explanation: "These categories are unusually high-impact and routinely indemnified.",
    recommendation: "Add an indemnity for damages arising from gross negligence, wilful misconduct, or breach of data-protection obligations.",
    present_patterns: [/(?:indemnif\w+).{0,200}(?:gross\s+negligence|wil[l]?ful\s+misconduct|data\s+protection|personal\s+data|breach\s+of\s+(?:DPA|data\s+protection))/is],
    default_severity: "warning",
  }),
  language({
    id: "MSA-005",
    ...
    explanation: "Whether indemnification falls inside or outside the cap is the most contested cap-carve-out term. The report surfaces it for explicit review.",
    recommendation: "Confirm intent: keep indemnity inside the cap (lower vendor exposure), put it outside (higher customer protection), or place it under a supercap (compromise).",
    bad_patterns: [
  ...
  presence({
    ...
    present_patterns: [/(aggregate\s+liability|total\s+liability).{0,80}(?:not\s+exceed|capped\s+at|limited\s+to)/i, /(twelve\s+months\s+(?:of\s+)?fees|12\s*months\s+(?:of\s+)?fees)/i],
  }),
  presence({
    id: "MSA-007",
    name: "Liability cap carve-outs (fraud / wilful misconduct / IP indemnity)",
    description: "Liability cap must carve out fraud, wilful misconduct, IP indemnity, and breach of confidentiality.",
    citation: "Commercial drafting baseline — cap carve-outs",
    missing_title: "Cap carve-outs missing",
    missing_description: "No carve-outs from the liability cap (fraud / wilful misconduct / IP indemnity / confidentiality / DP) were found.",
    explanation: "A cap that absorbs fraud and wilful misconduct is unconscionable in many jurisdictions and commercially abnormal.",
    recommendation: "Carve fraud, wilful misconduct, IP indemnification, confidentiality breach, and data-protection breach out of the cap.",
    present_patterns: [/(cap|limitation).{0,200}(?:shall\s+not\s+apply|excluded|carved\s+out).{0,200}(fraud|wil[l]?ful\s+misconduct|IP\s+indemn|confidentiality|data\s+protection)/is, /supercap\b/i],
    default_severity: "warning",
  }),
  presence({
    id: "MSA-008",
    ...
    missing_description: "Consequential-damages language was found but no mutual / either-party scoping was detected.",
    explanation: "An asymmetric waiver lets one side recover lost-profit damages and bars the other; the mutual form is the commercial baseline.",
    recommendation: "Phrase the waiver as 'neither party shall be liable to the other for any indirect, incidental, special, consequential, or punitive damages.'",
    present_patterns: [/(?:neither\s+party|each\s+party|in\s+no\s+event\s+shall\s+either\s+party).{0,160}(?:consequential|indirect|special|punitive|lost\s+profits)/is],
    default_severity: "info",
  }),
  language({
    id: "MSA-009",
    name: "California Civil Code § 1668 problem flag",
    description: "Flags a liability cap or exculpation that purports to limit liability for fraud, wilful injury, or violation of law where California law may govern — void per Cal. Civ. Code § 1668.",
    citation: "California Civil Code § 1668",
    bad_title: "Cap may run afoul of Cal. Civ. Code § 1668",
    bad_description: "Liability limitation purports to limit fraud / wilful injury / violation of law and California law may govern.",
    explanation: "California Civil Code § 1668 voids contracts that exempt anyone from responsibility for their own fraud, wilful injury, or violation of law. A cap that absorbs these categories is unenforceable in California.",
    recommendation: "Carve fraud, wilful injury, and violations of law out of any liability cap; verify governing law.",
    bad_patterns: [
  ...
  language({
    ...
    description: "Flags a broad indemnification for negligence in a construction-related MSA governed by New York law — void per N.Y. Gen. Oblig. § 5-322.1.",
    citation: "N.Y. Gen. Oblig. Law § 5-322.1",
    bad_title: "Indemnity for own negligence under NY § 5-322.1",
    bad_description: "Indemnification appears to require one party to indemnify the other for the indemnitee's own negligence in a construction context.",
    explanation: "N.Y. Gen. Oblig. § 5-322.1 voids construction-contract indemnities that require the indemnitor to indemnify the indemnitee for the indemnitee's own negligence.",
    recommendation: "Narrow the indemnity to the indemnitor's own negligence (or its share under comparative-negligence apportionment).",
    bad_patterns: [
  ...
  presence({
    ...
    description: "MSA must allocate ownership of background IP (pre-existing) and foreground IP (created during the engagement).",
    ...
    explanation: "Without allocation, default copyright / patent rules govern, which rarely matches the parties' intent.",
    recommendation: "Add explicit allocation: each party retains its background IP; foreground IP ownership rule (usually customer for deliverables, vendor for tooling).",
    present_patterns: [/(background\s+(?:IP|intellectual\s+property)|pre[- ]existing\s+IP)/i, /(foreground\s+(?:IP|intellectual\s+property)|developed\s+(?:hereunder|under\s+this\s+Agreement))/i],
    default_severity: "warning",
  }),
  language({
    id: "MSA-012",
    name: "Feedback license may be unbounded",
    description: "Flags a feedback license that conveys unrestricted, perpetual, irrevocable rights in customer feedback without scope limits.",
    citation: "Commercial drafting baseline — feedback license scope",
    bad_title: "Feedback license unbounded",
    bad_description: "Feedback license appears to grant unlimited, perpetual, irrevocable rights without scope limits.",
    explanation: "A broad feedback grant can sweep in customer ideas the customer may want to commercialize separately.",
    recommendation: "Limit the feedback license to the vendor's product improvement and add 'non-confidential feedback' as the trigger; consider a scope cap.",
    bad_patterns: [
  ...
  presence({
    ...
    description: "MSA must include workmanlike-services / conformance-to-documentation / no-malicious-code warranties.",
    citation: "Commercial drafting baseline — service warranties",
    missing_title: "Service warranties incomplete",
    missing_description: "Standard service warranties (workmanlike, conformance to documentation, no malicious code) were not found.",
    explanation: "These are the commercial baseline warranties for a services MSA.",
    recommendation: "Add: services performed in a workmanlike manner, conforming to the documentation, free of malicious code.",
    present_patterns: [/(workmanlike|professional\s+manner)/i, /(conform\w*\s+(?:with|to)\s+the\s+(?:documentation|specifications))/i, /(no\s+(?:malicious\s+code|virus|worm|trojan|disabling\s+device))/i],
    default_severity: "warning",
  }),
  presence({
    id: "MSA-014",
    ...
    recommendation: "Add: services and deliverables (a) comply with applicable laws and (b) do not infringe any third-party IP rights.",
    present_patterns: [/(?:comply|compliance).{0,40}(?:applicable\s+laws|laws\s+and\s+regulations)/i, /(?:do\s+not\s+(?:infringe|violate)|non[- ]infring\w+)/i],
    default_severity: "warning",
  }),
  language({
    id: "MSA-015",
    name: "Implied-warranty disclaimer overreach (UCC alignment)",
    description: "Flags an implied-warranty disclaimer that may overreach the UCC's conspicuous-disclaimer requirement.",
    citation: "U.C.C. § 2-316",
    bad_title: "Implied-warranty disclaimer may overreach UCC § 2-316",
    bad_description: "Implied-warranty disclaimer is present but may not satisfy UCC § 2-316's conspicuous-disclaimer requirement (or may include warranties UCC § 2-316 reserves).",
    explanation: "UCC § 2-316 requires merchantability disclaimers to mention 'merchantability' and to be conspicuous; many MSAs default to a generic 'AS IS' that fails the test.",
    recommendation: "Use the UCC-safe disclaimer: '[ALL-CAPS] VENDOR DISCLAIMS ALL IMPLIED WARRANTIES, INCLUDING MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT.' Confirm conspicuousness.",
    bad_patterns: [
  ...
  presence({
    ...
    explanation: "An MSA for hosted services without an SLA leaves availability promises unenforceable.",
    recommendation: "Reference an SLA (attached or linked) with uptime, support response, and remedy schedules.",
  ...
  language({
    ...
    bad_description: "Service-level credit appears to be the sole and exclusive remedy for downtime.",
    explanation: "An exclusive-remedy SLA bars the customer from terminating or seeking damages for extended outages.",
    recommendation: "Add a 'chronic failure' termination right after N consecutive months of breach, and preserve material-breach termination.",
    bad_patterns: [
  ...
  presence({
    ...
    explanation: "Without an explicit material-breach termination right the customer is forced into a common-law theory.",
    recommendation: "Add: either party may terminate on N (e.g., 30) days' written notice of an uncured material breach.",
    present_patterns: [/(material\s+breach|materially\s+breach\w*).{0,160}(?:cure|notice|terminate)/is],
    default_severity: "warning",
  }),
  presence({
    id: "MSA-019",
    ...
    explanation: "11 U.S.C. § 365 makes pure ipso-facto clauses unenforceable in some scenarios, but the contract should still anchor the parties' intent.",
    recommendation: "Add: termination on filing for bankruptcy, appointment of receiver, or assignment for the benefit of creditors.",
    present_patterns: [/(bankruptc\w+|insolven\w+|receiver|assignment\s+for\s+the\s+benefit\s+of\s+creditors)/i],
    default_severity: "info",
  }),
  presence({
    id: "MSA-020",
    ...
    missing_description: "No transition / wind-down / hosted-service continuation period was found.",
    explanation: "Without a wind-down, the customer can be cut off mid-migration.",
    recommendation: "Add: vendor will continue providing the service for up to N (e.g., 90) days post-termination at then-current rates to facilitate transition.",
    present_patterns: [/(wind[- ]down|transition\s+(?:assistance|services|period)|continued\s+access|post[- ]termination\s+(?:access|services))/i],
    default_severity: "info",
  }),
  ...
  presence({
    ...
    explanation: "Without an explicit return obligation the customer can be locked out of its own data.",
    recommendation: "Add: on termination, vendor shall return all customer data in a machine-readable format and then delete its copies within N days.",
    present_patterns: [/(return\s+(?:all\s+)?(?:customer\s+)?data|data\s+portability|export\s+(?:in\s+)?(?:a\s+)?machine[- ]readable)/i],
    default_severity: "warning",
  }),
  ...
  presence({
    ...
    description: "Force-majeure clause should apply to both parties symmetrically; payment obligations should be excluded.",
    ...
    explanation: "A one-sided force-majeure clause (vendor-only) is commercially abnormal; payment obligations are conventionally excluded.",
    recommendation: "Add: 'neither party shall be liable for delay or failure due to force majeure', with a payment-obligation carve-out.",
    present_patterns: [/(neither\s+party|either\s+party).{0,80}force\s+majeure/is, /force\s+majeure.{0,160}(?:neither|both|each\s+party)/is],
    default_severity: "info",
  }),
  ...
  language({
    ...
    bad_description: "An assignment clause is present but does not address change-of-control or merger.",
    explanation: "Without a change-of-control hook, an acquirer can effectively step into the contract without the counterparty's consent.",
    recommendation: "Add: 'a change of control of either party shall be deemed an assignment requiring consent (or notice)' with an affiliate carve-out as desired.",
    bad_patterns: [
  ...
  language({
    ...
    explanation: "Choosing one state's law but another state's forum forces the forum court to apply foreign law — adds cost and uncertainty.",
    recommendation: "Either align the two or document the rationale (e.g., NY law / Delaware courts for incorporated entities).",
    bad_patterns: [
  ...
  presence({
    ...
    explanation: "Without these, oral modifications and unintentional waivers become litigation risks.",
    recommendation: "Add: 'no amendment is effective unless in writing signed by both parties' and 'no failure to enforce shall be deemed a waiver.'",
    present_patterns: [/amend\w+.{0,40}(?:in\s+writing|written\s+(?:and\s+)?signed)/is, /(?:no\s+(?:waiver|failure)|shall\s+not\s+be\s+deemed\s+a\s+waiver)/i],
    default_severity: "info",
  }),
  presence({
    id: "MSA-026",
    ...
    explanation: "Without survival, sticky obligations (confidentiality, IP, indemnity) may not outlive termination. Without integration, prior negotiations may be admissible.",
    recommendation: "Add: a survival clause enumerating sticky obligations and an entire-agreement clause superseding prior negotiations.",
    present_patterns: [/(surviv\w+).{0,40}termination/is, /entire\s+agreement|integration\s+clause|supersed\w+\s+(?:all\s+)?prior/i],
    default_severity: "info",
  }),
  ...
  presence({
    ...
    description: "MSA should address whether the vendor uses AI / generative AI in providing the service.",
    citation: "NIST AI RMF — transparency baseline",
    missing_title: "AI usage clause missing",
    missing_description: "No clause was found addressing AI / generative AI usage by the vendor in service delivery.",
    explanation: "Increasingly the contract should disclose AI use, training data, and IP ownership of outputs (see the §49 AI Addendum playbook for the deep check).",
    recommendation: "Add a clause disclosing whether AI is used, naming material AI subprocessors, and addressing IP in outputs.",
    present_patterns: [/(artificial\s+intelligence|generative\s+AI|foundation\s+model|large\s+language\s+model|\bLLM\b)/i],
    default_severity: "info",
  }),
  ...
  language({
    ...
    description: "Flags an indemnity for the indemnitee's own negligence in a Texas-governed construction MSA — void per Tex. Bus. & Com. Code § 151.102.",
    citation: "Tex. Bus. & Com. Code § 151.102",
    bad_title: "Indemnity may violate Texas anti-indemnity statute",
    bad_description: "Indemnification appears to require one party to indemnify the other for the indemnitee's negligence in a construction-related MSA governed by Texas law.",
    explanation: "Texas Bus. & Com. Code Ch. 151 voids construction-contract indemnities that cover the indemnitee's own negligence (with limited insurance-policy exceptions).",
    recommendation: "Narrow to the indemnitor's own negligence; verify governing law and project-state nexus.",
    bad_patterns: [
  ...
  presence({
    ...
    description: "When a limited remedy is the exclusive remedy, the MSA should anchor the UCC § 2-719(2) escape if the remedy fails of its essential purpose.",
    citation: "U.C.C. § 2-719(2)",
    missing_title: "UCC § 2-719 limited-remedy escape missing",
    missing_description: "Exclusive / limited remedy language is present but no UCC § 2-719(2) fail-of-essential-purpose escape was found.",
    explanation: "UCC § 2-719(2) preserves alternative remedies when a limited remedy fails of its essential purpose — anchoring this protects the customer from a remedy gap.",
    recommendation: "Add: 'if the limited remedy is found to fail of its essential purpose, the customer's other remedies under this Agreement and applicable law remain available.'",
    present_patterns: [/(fail\w*\s+of\s+(?:its\s+)?essential\s+purpose|essential\s+purpose|U\.?C\.?C\.?\s*§\s*2-719)/i],
    default_severity: "info",
  }),
];
