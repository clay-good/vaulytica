...
const CONFIG_SCC: RegulatedRuleConfig = {
  ...
  cite_for(citation: string) {
    ...
    return { id: `transfer-${citation.replace(/[^A-Za-z0-9]+/g, "-").toLowerCase()}`, source_url: url };
  },
};

const CONFIG_UK: RegulatedRuleConfig = { ...CONFIG_SCC, applies_to_playbooks: UK_PLAYBOOKS };
const CONFIG_ALL: RegulatedRuleConfig = { ...CONFIG_SCC, applies_to_playbooks: ALL_TRANSFER_PLAYBOOKS };
...
export const TRANSFER_RULES: Rule[] = [
  ...
  presence({
    ...
    explanation: "Decision 2021/914 requires Clause 1 (Purpose and Scope) — it sets out the parties' agreement to be bound by the SCCs.",
  ...
  presence({
    ...
    explanation: "Clause 2 forbids material modification — the parties may add other clauses or business-related terms only if they do not contradict the SCCs.",
  ...
  language({
    ...
    description: "Flags any 'as modified' / 'notwithstanding' / 'as amended' language attached to SCC clauses — forbidden by Clause 2.",
    citation: "EU SCCs Clause 2",
    bad_title: "SCC clauses appear materially modified",
    bad_description: "Detected modification language attached to SCC clauses — forbidden by Clause 2.",
    explanation: "Clause 2 forbids modifying the SCC text. 'As modified', 'notwithstanding the SCCs', 'except for', etc. are all red flags.",
    recommendation: "Remove any modifying language; place business terms in a separate Annex or Side Letter that does not contradict the SCCs.",
    bad_patterns: [
  ...
  presence({
    ...
    explanation: "Clause 8 contains the bulk of the Module-2 data-protection obligations (instructions, purpose limitation, transparency, accuracy, minimisation, storage limitation, security).",
  ...
  presence({
    ...
    recommendation: "Include the full Clause 9 text and complete Annex III (List of Sub-processors).",
  ...
  presence({
    ...
    explanation: "Clause 11 obligates the data importer to inform data subjects of redress mechanisms.",
  ...
  presence({
    ...
    description: "SCC Clause 14 (Local laws and practices affecting compliance) must be present, anchoring the TIA.",
    ...
    present_patterns: [/(clause\s*14\b|local\s+laws\s+and\s+practices|transfer\s+impact\s+assessment|\bTIA\b)/i],
  }),
  presence({
    id: "TRANSFER-008",
    name: "SCC Clause 15 — Public Authority Access",
    description: "SCC Clause 15 (Obligations of the data importer in case of public authority access) must be present.",
    ...
    explanation: "Clause 15 requires the data importer to notify and challenge public-authority requests where possible.",
    recommendation: "Include the full Clause 15 text unmodified.",
    present_patterns: [/(clause\s*15\b|public\s+authority\s+(?:access|request)|government\s+access\s+request|law\s+enforcement\s+request)/i],
  }),
  presence({
    id: "TRANSFER-009",
  ...
  presence({
    ...
    description: "SCC Clause 18 (Governing Law / Choice of Forum and Jurisdiction) must be present.",
    ...
    explanation: "Clause 18 sets the governing law (must be an EU Member State allowing third-party-beneficiary rights) and forum.",
    recommendation: "Include the full Clause 18 text and pick a qualifying EU Member State.",
    present_patterns: [/(clause\s*18\b|governing\s+law\s+and\s+(?:forum|jurisdiction)|choice\s+of\s+forum)/i],
  }),
  ...
  presenceUk({
    ...
    present_patterns: [/Table\s*2\s*[—:-]\s*(?:Selected\s+)?SCC\s+Modules|table\s*2\b.{0,40}module/i],
    default_severity: "warning",
  }),
  presenceUk({
    id: "TRANSFER-013",
    ...
    recommendation: "Complete Table 3 with the SCC Annex information (parties, transfer, supervisory authority, TOMs).",
  ...
  presenceUk({
    ...
    description: "UK Addendum Table 4 (Ending This Addendum When the Approved Addendum Changes) must be completed.",
    ...
    recommendation: "Complete Table 4 indicating which party (or both) may end the Addendum on a revision.",
    present_patterns: [/Table\s*4\s*[—:-]\s*Ending|table\s*4\b.{0,40}(ending|approved\s+addendum)/i],
    default_severity: "warning",
  }),
  languageUk({
    id: "TRANSFER-015",
    ...
    bad_description: "Detected modification language attached to the UK Addendum mandatory clauses.",
    explanation: "The ICO UK Addendum's Mandatory Clauses cannot be modified — only the Tables may be completed.",
  ...
  presenceUk({
    ...
    explanation: "The UK IDTA (standalone) requires Part 1 to identify the parties; the UK Addendum (layered on EU SCCs) uses Table 1 for the same purpose.",
    recommendation: "Complete Part 1 (IDTA) or Table 1 (Addendum) with the parties' legal entity names and addresses.",
    present_patterns: [/Part\s*1\s*[—:-]\s*Parties|part\s*1\b.{0,40}part/i, /Table\s*1\s*[—:-]\s*Parties|table\s*1\b.{0,40}part/i],
    default_severity: "warning",
  }),
  ...
  languageAll({
    ...
    description: "Flags reliance on the EU-US Data Privacy Framework (or UK extension) — a warning given pending litigation.",
    citation: "EU-US Data Privacy Framework",
    bad_title: "Reliance on litigation-pending adequacy decision (DPF)",
    bad_description: "Detected reliance on the EU-US Data Privacy Framework as the transfer mechanism.",
    explanation: "The EU-US Data Privacy Framework is the operative adequacy decision but is permanently under litigation. v3 treats this as a warning, not a fail.",
    recommendation: "Maintain a fallback transfer mechanism (e.g., SCCs + TIA) in case the DPF is invalidated again.",
    bad_patterns: [
  ...
  presenceAll({
    ...
    description: "Where an adequacy decision is relied on, the DPA should anchor the reliance with a fallback for invalidation.",
    citation: "GDPR Art. 45 / Adequacy decisions",
    missing_title: "Adequacy fallback clause missing",
    missing_description: "No fallback clause was found in case the adequacy decision is invalidated.",
    explanation: "Adequacy decisions can be invalidated (Schrems I, Schrems II). A fallback to SCCs / IDTA prevents transfer disruption.",
    recommendation: "Add: 'In the event the relied-upon adequacy decision is invalidated, the parties shall promptly implement the EU SCCs / UK Addendum / UK IDTA as a fallback.'",
    present_patterns: [/(adequacy\s+decision\s+is\s+(?:invalidated|revoked)|fallback\s+(?:to\s+)?SCC|substitute\s+(?:transfer\s+)?mechanism)/i],
    default_severity: "warning",
  }),
  ...
  presenceAll({
    ...
    description: "Where SCCs / IDTA cover transfers to a non-adequate country, the DPA must reference a TIA / TRA.",
    ...
    explanation: "EDPB Recommendations 01/2020 require the parties to assess local laws and practices of the recipient country.",
    recommendation: "Reference a TIA / TRA in the DPA / SCC Annex and document the supplementary measures where needed.",
    present_patterns: [/(transfer\s+(?:impact|risk)\s+assessment|\bTIA\b|\bTRA\b|local\s+laws\s+and\s+practices|supplementary\s+measures)/i],
  }),
  presenceAll({
    id: "TRANSFER-020",
    name: "Onward-transfer terms (Clause 8.7 / 8.8)",
    description: "Where SCCs apply, the DPA should address onward-transfer terms per SCC Clause 8.7 / 8.8.",
    ...
    explanation: "SCC Clause 8.7 / 8.8 governs onward transfers to third parties outside the EEA. For the UK Addendum (layered on EU SCCs), Clause 8.8 is incorporated by reference but explicit acknowledgment is best practice.",
