...
export const DPA_US_STATE_RULES: Rule[] = [
  ...
  presence({
    ...
    missing_description:
      "No clause was found prohibiting use of personal information outside the specific business purpose.",
    explanation:
      "§ 1798.140(ag)(1)(B) requires the contract to prohibit the service provider from retaining, using, or disclosing personal information for any purpose other than the specific business purpose enumerated.",
    recommendation:
  ...
  presence({
    ...
    missing_description:
      "No clause was found prohibiting combining personal information with other data sources.",
    explanation:
  ...
  presence({
    ...
    explanation:
      "§ 1798.100(d) requires the contract to obligate the third party / service provider to comply with applicable obligations and provide the same level of privacy protection as required by the CCPA.",
    recommendation:
  ...
  presence({
    ...
    missing_description:
      "No clause was found requiring subcontractors to meet the same CCPA obligations.",
    explanation:
      "§ 7051(b) requires the contract with subcontractors to include the same provisions.",
    recommendation:
  ...
  presence({
    ...
    missing_description:
      "No clause was found obligating the processor to cooperate with reasonable assessments.",
    explanation:
  ...
  presence({
    ...
    explanation:
      "Every state's processor-contract statute requires subcontractor engagement to be by written contract imposing equivalent obligations.",
    recommendation:
      "Add: 'Processor shall engage any subcontractor only pursuant to a written contract requiring the subcontractor to meet the obligations of Processor under this Agreement.'",
    present_patterns: [
      /(subcontractor|sub[- ]processor).{0,200}(written\s+contract|written\s+agreement|same\s+obligations)/is,
    ],
  }),
  presence({
    id: "USDPA-019",
    ...
    missing_description:
      "No clause was found requiring the processor to provide information demonstrating compliance.",
    explanation:
  ...
  language({
    ...
    bad_description:
      "Detected a multi-state DPA using minimum 'Utah-style' contract terms where stricter state law (CCPA, CPA, CTDPA) may apply.",
    explanation:
      "When a contract spans multiple states, the strictest applicable requirement should govern. Using minimum-common-denominator language risks non-compliance with the stricter states.",
    recommendation:
      "Adopt the strictest applicable state's required elements (typically CCPA / CPA / CTDPA) and document the multi-state coverage explicitly.",
    bad_patterns: [
  ...
  presence({
    ...
    missing_description:
      "No clause was found specifically addressing sensitive personal information.",
    explanation:
      "CCPA / CPRA, CPA, CTDPA define sensitive personal information / sensitive data and impose heightened obligations.",
    recommendation:
      "Add a 'Sensitive Personal Information' clause describing categories handled and SPI-specific safeguards.",
  ...
  presence({
    ...
    missing_description:
      "No clause was found documenting the process for consumer rights requests (access, deletion, opt-out).",
    explanation:
      "All US state privacy statutes provide consumer rights; the DPA should describe the operational handoff.",
    recommendation:
  ...
  presence({
    ...
    explanation:
      "Several state privacy statutes incorporate a data-minimization principle requiring collection of only what is reasonably necessary.",
    recommendation:
      "Add: 'Personal data collected and processed shall be limited to what is reasonably necessary and proportionate to the disclosed purpose.'",
    present_patterns: [
  ...
  presence({
    ...
    explanation:
      "A US DPA template that never mentions 'personal information' or 'personal data' is highly likely to be the wrong template.",
    recommendation:
      "Use a DPA template that explicitly references personal information / personal data throughout.",
...
if (DPA_US_STATE_RULES.length !== 25) {
  throw new Error(
    `DPA-US-state ruleset must export exactly 25 rules; got ${DPA_US_STATE_RULES.length}`,
  );
}
