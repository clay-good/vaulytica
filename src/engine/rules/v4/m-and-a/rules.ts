...
const LOI_TERM_SHEET_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [
      /expenses\s+(shall|will|must)\s+be\s+borne/i,
...
const SPA_RULES: Rule[] = [
  presence({
    ...
    present_patterns: [
      ...
      /buyer\s+(?:shall|will|must)\s+purchase.{0,40}shares?/is,
      /\bsells?\s+(?:and\s+(?:transfers?|delivers?|assigns?)\s+)?to\s+[A-Z]?[\w-]+\b[^.]{0,80}?\bshares?\b/,
      /\b[A-Z]?[\w-]+\s+(?:(?:shall|will|must)\s+|agrees?\s+to\s+)?purchases?\s+from\s+[A-Z]?[\w-]+\b[^.]{0,80}?\bshares?\b/,
    ],
  }),
  presence({
    id: "MNA-011",
  ...
  presence({
    ...
    present_patterns: [
      ...
      /buyer.s\s+knowledge.{0,80}(shall|will|must).{0,40}not\s+(affect|limit|impair)/is,
      /investigation.{0,40}(shall|will|must).{0,40}not\s+(affect|limit|impair)/is,
    ],
    default_severity: "warning",
  }),
  presence({
...
const MERGER_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [
      ...
      // The condition is as often phrased with the approval FIRST — "subject
...
const DISCLOSURE_SCHEDULE_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [
      /materiality.{0,40}(disclaim|(?:shall|will|must)\s+not\s+(be|imply))/is,
      /threshold.{0,40}(disclaim|(?:shall|will|must)\s+not\s+(be|imply))/is,
...
const ESCROW_AGREEMENT_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [/notices/i, /(?:shall|will|must)\s+be\s+delivered/i],
...
const EARNOUT_RULES: Rule[] = [
  ...
  language({
    ...
    bad_patterns: [
      ...
      /buyer\s+(may|(?:shall|will|must))\s+operate.{0,40}sole\s+discretion.{0,80}earn[-\s]?out/is,
    ],
...
const MA_RESTRICTIVE_COVENANT_RULES: Rule[] = [
  ...
  language({
    ...
    bad_patterns: [
      ...
      /(?:non.?compete|non.?competition|restricted\s+period)[^.]{0,40}(?:of|(?:shall|will|must)\s+be|is|are|equal\s+to|not\s+exceed(?:ing)?)\s+(?:a\s+period\s+of\s+)?(?:[a-z]+[-\s]+){0,2}\(?([6-9]|[1-9][0-9])\)?\s+years?/is,
      /(?:for|of)\s+(?:a\s+)?(?:period\s+of\s+)?(?:[a-z]+[-\s]+){0,2}\(?([6-9]|[1-9][0-9])\)?\s+years?[^.]{0,40}(?:non.?compete|non.?competition|restricted\s+period|(?:shall|will|must)\s+not\s+compete)/is,
      ...
      // forms) both missed.
      /(?:shall|will|must)\s+not\s+compete\b[^.]{0,30}(?:for|of|during)\s+(?:a\s+)?(?:period\s+of\s+)?(?:[a-z]+[-\s]+){0,2}\(?([6-9]|[1-9][0-9])\)?\s+years?/is,
    ],
