...
const NET_LEASE_RULES: Rule[] = [
  ...
  presence({
    ...
    denied_if: [
      ...
      /\bno\s+insurance\s+(?:is|(?:shall|will|must)\s+be)\s+required/i,
...
const CCR_RULES: Rule[] = [
  ...
  language({
    ...
    bad_patterns: [
      ...
      /\b(no|not|(?:shall|will|must)\s+not|nor)\b[^.]{0,90}\b(?:african|asian|black|hispanic|jewish|negro|caucasian|latino|latina|oriental|mongolian|semitic|persons?\s+of\s+colou?r|of\s+(?:the\s+)?(?:caucasian|white|negro|african|asian|hebrew|jewish|mongolian|malay)\s+(?:race|blood|descent|ancestry|extraction)|(?:african|asian|hebrew|mexican|chinese|japanese|jewish)\s+(?:descent|ancestry|origin|blood|extraction|faith|religion))\b/is,
    ...
    exclude_if: [
      /\bfair\s+housing\b/i,
      /(?:shall|will|must)\s+not\s+be\s+denied/i,
...
const SNDA_RULES: Rule[] = [
  presence({
    ...
    present_patterns: [
      ...
      /\b(?:mortgage|deed\s+of\s+trust|security\s+instrument)\b[^.]{0,60}?\bis\s+(?:and\s+(?:shall|will|must)\s+(?:at\s+all\s+times\s+)?(?:be|remain)\s+)?(?:senior|prior|superior)\s+to\b/is,
      /lease[^.]{0,60}?\b(?:is|(?:shall|will|must)\s+be|are)\b[^.]{0,40}?\bsub(?:ject\s+and\s+sub)?ordinate/is,
    ],
  }),
  presence({
    id: "RE-047",
    ...
    present_patterns: [
      ...
      /\b(?:shall|will|must)\s+not\s+disturb\b[^.]{0,80}?\b(?:possession|tenant|tenancy|leasehold)\b/is,
      /\bnot\s+(?:be\s+)?(?:terminated?|extinguished|disturbed|affected)\b[^.]{0,100}?\b(?:foreclosure|foreclos\w+|power\s+of\s+sale)\b/is,
      /\bforeclos\w+[^.]{0,100}?\b(?:lease|tenancy|leasehold)\b[^.]{0,60}?\b(?:(?:shall|will|must)\s+)?(?:continue|remain|survive)\b/is,
      ...
      /\b(?:shall|will|must)\s+not\b[^.]{0,140}?\bdisturb\b[^.]{0,90}?\b(?:possession|tenant|tenancy|leasehold|quiet\s+enjoyment)\b/is,
      ...
      // power of sale."
      /\b(?:lease|tenancy|leasehold|tenant['’]?s?\s+rights?)\b[^.]{0,100}?\b(?:(?:shall|will|must)\s+)?(?:continue|remain|survive)\b[^.]{0,140}?\b(?:foreclos\w+|power\s+of\s+sale|deed\s+in\s+lieu)\b/is,
    ],
  }),
  presence({
    id: "RE-048",
...
const LEASE_ASSIGNMENT_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [
      ...
      /assignor[^.]{0,120}?(?:remains?|(?:shall|will|must)\s+remain|continues?\s+to\s+be|(?:shall|will|must)\s+continue\s+to\s+be)\s+liable|continuing\s+liabilit(?:y|ies)|assignor\s+is\s+not\s+(?:released|relieved)|(?:does\s+not|(?:shall|will|must)\s+not)\s+(?:release|relieve)\s+(?:the\s+)?assignor/is,
    ],
    default_severity: "warning",
  }),
  presence({
