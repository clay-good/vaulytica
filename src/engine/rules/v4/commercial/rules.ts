...
const DISTRIBUTION_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [
      /(distributor|reseller)\s+(may|(?:shall|will|must))\s+(determine|set|establish).{0,30}(resale\s+)?price/is,
  ...
  presence({
    ...
    present_patterns: [
      ...
      /(?:shall|will|must)\s+not\s+(carry|sell|distribute|promote)[\s\S]{0,40}(competing|competitive)/is,
      /non.?compet\w*/i,
  ...
  presence({
    ...
    denied_if: [
      /\b(?:does|do|shall|will|must)\s+not\s+(?:pass|flow|extend|assign)\b[^.]{0,60}?\bwarrant\w*/i,
      /\bno\s+warrant\w*\s+(?:is|are|(?:shall|will|must)\s+be)\s+(?:passed|flowed|extended|assigned)/i,
...
const REFERRAL_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [
      ...
      /(?:shall|will|must)\s+not\s+.{0,30}(bind|obligate|make\s+.{0,10}representations)/is,
    ],
  }),
  presence({
    id: "COMM-018",
  ...
  presence({
    ...
    present_patterns: [
      ...
      /(?:shall|will|must)\s+not\s+(bypass|circumvent|solicit\s+directly)/i,
  ...
  presence({
    ...
    present_patterns: [
      ...
      /(termination|expiration)[\s\S]{0,80}(referral\s+fee|fee\s+(?:shall|will|must)\s+(still\s+)?be\s+(paid|payable)|earned\s+prior)/is,
  ...
  presence({
    ...
    present_patterns: [
      ...
      /(?:shall|will|must)\s+not\s+.{0,40}(misrepresent|make\s+false)/is,
...
const MARKETING_RULES: Rule[] = [
  ...
  presence({
    ...
    present_patterns: [
      ...
      /(deliverables?|work\s+product)\s+.{0,50}(owned\s+by|(?:shall|will|must)\s+be\s+owned|belong|are\s+owned|vest)/is,
  ...
  presence({
    ...
    denied_if: [
      /\b(?:does|do|shall|will|must)\s+not\s+(?:obtain|secure|clear)\b[^.]{0,60}?\b(?:rights|licen[cs]es?|releases?|clearances?|permissions?)/i,
      /\b(?:makes?|gives?|provides?)\s+no\b[^.]{0,40}?\bnon.?infringement\b/i,
      /\bno\s+(?:clearance|rights?.clearance)\s+(?:is|are|(?:shall|will|must)\s+be)\s+(?:obtained|provided|required)/i,
    ],
  ...
  presence({
    ...
    present_patterns: [
      ...
      /non.?disclosure|\bnda\b/i,
      /(?:shall|will|must)\s+not\s+(disclose|use)[\s\S]{0,40}confiden\w*/is,
    ],
    ...
    denied_if: [
      ...
      /\bconfidentiality\s+obligations?\b[^.]{0,40}?\b(?:do|does|(?:shall|will|must))\s+not\s+apply\s+to\s+(?:this\s+)?(?:agreement|the\s+parties)/i,
    ],
  ...
  presence({
    ...
    present_patterns: [
      ...
      /(subcontractor|influencer)s?[\s\S]{0,40}((?:shall|will|must)\s+)?(comply|be\s+bound|agree)/is,
    ],
  }),
];
