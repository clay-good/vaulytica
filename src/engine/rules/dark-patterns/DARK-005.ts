...
export const rule: Rule = {
  ...
  check(ctx: RuleContext): Finding | null {
    const hit = firstParagraphMatch(
      ...
      // form.
      /\b(?:waives?|waiv(?:er|ing)|gives?\s+up|relinquishes?|(?:shall|must)\s+not\s+(?:participate|be\s+entitled))\b[^.]{0,200}\b(?:class\s+action|class[-\s]wide|collective\s+action|representative\s+(?:action|proceeding)|class\s+or\s+representative\s+(?:action|proceeding|arbitration)|consolidated\s+(?:claims?|arbitration))\b|\b(?:no|not)\s+(?:class\s+action|class[-\s]wide|collective\s+action|representative\s+(?:action|proceeding))\b(?!\s+waiver)|\bon\s+an\s+individual\s+basis\s+(?:only|and\s+not)\b|\b(?:an?\s+|your\s+|its\s+|his\s+|her\s+|their\s+)?individual\s+capacity\b[^.]{0,90}?\bnot\s+as\s+(?:a\s+)?(?:plaintiff|class\s+member|representative|part\s+of\s+(?:a\s+)?class)\b|\bnot\s+as\s+a\s+(?:plaintiff\s+or\s+)?class\s+member\b|\b(?:may\s+not|shall\s+not|cannot|will\s+not)\s+(?:consolidate|preside\s+over|hear|entertain)\b[^.]{0,70}?\b(?:class|representative|collective)\s+(?:action|proceeding|claims?|arbitration)\b/i,
    );
    if (!hit) return null;
