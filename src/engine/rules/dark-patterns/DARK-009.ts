...
export const rule: Rule = {
  ...
  check(ctx: RuleContext): Finding | null {
    ...
    const a = firstParagraphMatch(
      ...
      /\b(?:Vendor|Provider|Company|Licensor|Customer|Operator|we|we\s+reserve\s+the\s+right\s+to)\s*(?:may|reserves?\s+the\s+right\s+to|(?:shall|will|must)\s+have\s+the\s+right\s+to)?\s*(?:modify|amend|change|update|revise|alter)\b[\s\S]{0,160}\b(?:terms?|agreement|service|policy|policies|conditions)\b[\s\S]{0,160}\b(?:post(?:ing)?|publish(?:ing)?|mak(?:e|es|ing)\s+[^.]{0,40}?available|made\s+[^.]{0,40}?available|upload(?:ing)?|placing)\b[\s\S]{0,80}\b(?:website|site|portal|url|link|page|online|on\s+its\s+(?:website|site|portal)|at\s+(?:the\s+)?url)\b/i,
    );
