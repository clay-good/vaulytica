...
export const rule: Rule = {
  ...
  check(ctx: RuleContext): Finding | null {
    const hit = firstParagraphMatch(
      ctx,
      // The forfeiture is also written "the deposit shall be RETAINED / KEPT /
      // HELD by Landlord as liquidated damages / upon breach / and not
      // returned" — a genuine forfeiture the "retained in full" branch missed.
      // It is anchored to a forfeiture context so a normal "held by Landlord
      // during the term" deposit-holding clause is not flagged.
      /\b(?:security\s+deposit|deposit)\b[^.]{0,80}\b(?:is\s+(?:non-?refundable|forfeited|not\s+refundable|retained\s+in\s+full)|shall\s+be\s+forfeited|will\s+not\s+be\s+returned|Tenant\s+(?:forfeits|waives\s+any\s+(?:right\s+to|claim\s+to)))\b|\b(?:security\s+deposit|deposit)\b[^.]{0,80}\b(?:retained|kept|held)\s+by\s+(?:the\s+)?(?:Landlord|Lessor|Owner)\b[^.]{0,70}\b(?:as\s+(?:liquidated\s+damages|a\s+penalty|compensation|its\s+property)|upon\s+(?:any\s+)?(?:breach|default|early\s+(?:termination|move))|and\s+(?:shall\s+)?not\s+(?:be\s+)?returned|in\s+full)\b|\bTenant\s+waives?\b[^.]{0,60}\b(?:itemi[sz]ed|written\s+(?:accounting|statement)|return)\b[^.]{0,40}\bdeposit|\bnon-?refundable\b[^.]{0,40}\bdeposit\b/i,
    );
