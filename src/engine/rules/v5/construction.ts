...
const DESIGN_BUILD = pack("design-build-agreement", C, [
  {
    ...
    pat: [
      /(single\s+point\s+of\s+responsibility|design-?builder\s+((?:shall|will|must)\s+be\s+)?(solely\s+)?responsible\s+for\s+both)/i,
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(approv|comment|(?:shall|will|must)\s+not\s+relieve|within\s+${PERIOD_COUNT}\s*days)`,
        "i",
      ),
    ],
...
const ARCHITECT = pack("architect-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(licen[cs]e|copyright|terminat|nonexclusive\s+licen[cs]e|(?:shall|will|must)\s+not\s+use)/i,
    ],
...
const PRELIEN = pack("preliminary-lien-notice", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(certified\s+mail|return\s+receipt|within\s+${PERIOD_COUNT}\s*days|20\s*\)?\s*days|proof\s+of\s+service)`,
        "i",
      ),
    ],
