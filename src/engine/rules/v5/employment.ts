...
const ARBITRATION = pack("arbitration-agreement-employment", C, [
  ...
  },
  {
    ...
    pat: [
      /(the\s+(company|employer)\s+(shall|will|must)\s+pay|at\s+the\s+(company['’]?s?|employer['’]?s?)\s+(sole\s+)?(cost|expense))/i,
  ...
  },
  {
    ...
    pat: [
      /(arbitrator\s+((?:shall|will|must)\s+be\s+)?(selected|chosen|appointed))/i,
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(within\s+${PERIOD_COUNT}\s*days|consideration|continued\s+employment|no\s+(adverse|retaliat))`,
        "i",
      ),
    ],
...
const COMMISSION = pack("commission-plan", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(commission|trailing|(?:shall|will|must)\s+(not\s+)?be\s+(paid|entitled))/i,
    ],
    all: true,
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(prospective(ly)?|upon\s+${PERIOD_COUNT}\s*days['’]?\s+notice|at\s+any\s+time|in\s+its\s+(sole\s+)?discretion)`,
        "i",
      ),
    ],
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(dispute|object|within\s+${PERIOD_COUNT}\s*days\s+of\s+receipt|inquir)`,
        "i",
      ),
    ],
...
const RELOCATION = pack("relocation-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(no\s+repayment|(?:shall|will|must)\s+not\s+(be\s+)?(required|obligated)\s+to\s+repay|waive)/i,
    ],
...
const REMOTE = pack("remote-work-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(at\s+any\s+time|upon\s+${PERIOD_COUNT}\s*days['’]?\s+notice|return\s+to\s+the\s+office|in\s+its\s+discretion)`,
        "i",
      ),
    ],
