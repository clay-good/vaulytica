...
const SUBLEASE = pack("sublease-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      // The consent is almost never named in that bare adjacency: a sublease
      // asks for the landlord's PRIOR WRITTEN consent, or conditions itself on
  ...
  },
  {
    ...
    pat: [/attorn/i, /(direct\s+lease|recogni[sz]e\s+the\s+subtenant|non-?disturbance)/i],
...
const PROPERTY_MGMT = pack("property-management-agreement", C, [
  ...
  },
  {
    ...
    pat: [/licen[cs](e|ed|ing)/i, /(real\s+estate\s+broker|brokerage|in\s+the\s+state\s+of)/i],
...
const LISTING = pack("listing-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(authori[sz]|photograph|lockbox|signage|yard\s+sign|internet\s+display|market\s+the\s+property)/i,
    ],
    all: true,
