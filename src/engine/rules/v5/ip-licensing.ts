...
const COEXISTENCE = pack("trademark-coexistence-agreement", C, [
  {
    ...
    pat: [
      ...
      /(shall\s+(only\s+)?use|limited\s+to|shall\s+not\s+use\s+the\s+mark\s+(on|for))/i,
    ],
  ...
  },
  {
    ...
    pat: [
      /(confusion)/i,
      /(shall\s+(notify|cooperate|take\s+(reasonable\s+)?steps)|disclaimer|misdirected\s+(inquiries|orders)|distinguish)/i,
    ],
...
const JDA = pack("joint-development-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(shall\s+(be\s+)?own(ed)?|title\s+to|jointly\s+owned|sole(ly)?\s+owned)/i,
    ],
  ...
  },
  {
    ...
    pat: [
      /exclusiv/i,
      /(field\s+of\s+use|shall\s+not\s+(develop|engage|work\s+with)|competing\s+(program|product)|during\s+the\s+term)/i,
    ],
...
const ESCROW = pack("source-code-escrow-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      /verif/i,
      /(integrity|compil|build\s+test|escrow\s+agent\s+(shall|may)\s+(verify|test)|beneficiary\s+may\s+request)/i,
    ],
