...
const DATA_SHARING = pack("data-sharing-agreement", C, [
  {
    ...
    pat: [
      ...
      /(solely\s+for|permitted\s+purpose|shall\s+(only\s+)?be\s+used\s+for|limited\s+to)/i,
    ],
  ...
  },
  {
    ...
    pat: [
      /re-?identif/i,
      /(shall\s+not|prohibit|attempt\s+to\s+(identify|re-?identify)|contact\s+(the\s+)?(individuals|subjects))/i,
    ],
