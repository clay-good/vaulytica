...
const DO = pack("do-policy", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(carve-?back|(?:do|does|shall|will|must)\s+not\s+apply\s+to|derivative\s+(action|demand)|bankruptcy\s+trustee|whistleblower|former\s+(director|officer))/i,
    ],
  ...
  },
  {
    ...
    pat: [
      /severab/i,
      /(application|rescission|imput|knowledge\s+of\s+one\s+insured|(?:shall|will|must)\s+not\s+be\s+imputed)/i,
    ],
