...
const INFOSEC = pack("information-security-policy", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(scan|remediat|severity|critical|within\s+${PERIOD_COUNT}\s*days|sla)`,
        "i",
      ),
    ],
...
const AUP = pack("acceptable-use-policy", C, [
  ...
  },
  {
    ...
    pat: [
      /(prohibit|may\s+not|(?:shall|will|must)\s+not|unacceptable\s+use)/i,
  ...
  },
  {
    ...
    pat: [
      ...
      /(nothing\s+in\s+this\s+policy|(?:shall|will|must)\s+not\s+be\s+(construed|interpreted)\s+to\s+(prohibit|restrict)|wages,?\s+hours,?\s+(and|or)\s+(other\s+)?(terms|working\s+conditions))/i,
    ],
...
const IRP = pack("security-incident-response-plan", C, [
  ...
  },
  {
    ...
    pat: [
      /(notif(y|ication))/i,
      new RegExp(
        String.raw`(72\s*\)?\s*hours|${PERIOD_COUNT}\s*(business\s+)?days|regulator|attorney\s+general|state\s+breach|hhs|securities\s+and\s+exchange|deadline)`,
        "i",
      ),
    ],
    all: true,
  ...
  },
  {
    ...
    pat: [
      ...
      new RegExp(
        String.raw`(review|within\s+${PERIOD_COUNT}\s*days|update\s+the\s+plan|corrective\s+action|root\s+cause)`,
        "i",
      ),
    ],
