...
const PURCHASE_ORDER = pack("purchase-order-terms", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(risk\s+of\s+loss|title\s+((?:shall|will|must)\s+)?pass)/i,
    ],
...
const MASTER_PURCHASE = pack("master-purchase-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(price\s+(list|schedule)|pricing\s+set\s+forth|(?:unit\s+)?prices?\b[^.]{0,30}?\b(?:are|is|shall\s+be|will\s+be|must\s+be)\s+(?:those\s+|the\s+ones\s+|as\s+)?(?:set\s+forth|stated|listed|specified|quoted)|(?:unit\s+)?prices?\s+(?:set\s+forth|stated|listed|specified|quoted)\b|prices?\b[^.]{0,30}?\b(?:in|on)\s+(?:the\s+)?(?:applicable\s+|each\s+)?(?:Exhibit|Schedule|Appendix|Annexure|Annex|purchase\s+order|order\s+form))/i,
...
const EQUIPMENT_LEASE = pack("equipment-lease", C, [
  {
    ...
    pat: [
      ...
      /(lessee['’]?s?\s+(?:\w+\s+){0,4}?obligations?\b[^.]{0,40}?\b(?:is|are|(?:shall|will|must)\s+be)\s+absolute\s+and\s+unconditional)/i,
    ],
...
const FREIGHT = pack("freight-transportation-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /((?:shall|will|must)\s+(control|govern)|order\s+of\s+precedence|conflict\s+between)/i,
    ],
...
const TEAMING = pack("teaming-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      /exclusiv/i,
      /((?:shall|will|must)\s+not\s+(team|participate)\s+with|other\s+offerors?|competing\s+team)/i,
    ],
...
const INSERTION_ORDER = pack("advertising-insertion-order", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(agency\s+is\s+not\s+liable|solely\s+as\s+agent|advertiser\s+(?:shall|will|must)\s+be\s+liable)/i,
    ],
...
const INFLUENCER = pack("influencer-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(may\s+not\s+(make|state)|(?:shall|will|must)\s+only\s+(make|use)|talking\s+points)/i,
    ],
  ...
  },
  {
    ...
    pat: [
      /exclusiv/i,
      /(competing\s+(brand|product)|(?:shall|will|must)\s+not\s+promote|category)/i,
    ],
...
const MEDIA_RELEASE = pack("media-release", C, [
  ...
  },
  {
    ...
    pat: [
      /(irrevocab|(?:shall|will|must)\s+not\s+revoke)/i,
