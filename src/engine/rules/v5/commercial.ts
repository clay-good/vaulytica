...
const MASTER_PURCHASE = pack("master-purchase-agreement", C, [
  ...
  },
  {
    ...
    pat: [
      ...
      /(price\s+(list|schedule)|pricing\s+set\s+forth|(?:unit\s+)?prices?\b[^.]{0,30}?\b(?:are|is|shall\s+be|will\s+be)\s+(?:those\s+|the\s+ones\s+|as\s+)?(?:set\s+forth|stated|listed|specified|quoted)|(?:unit\s+)?prices?\s+(?:set\s+forth|stated|listed|specified|quoted)\b|prices?\b[^.]{0,30}?\b(?:in|on)\s+(?:the\s+)?(?:applicable\s+|each\s+)?(?:Exhibit|Schedule|Appendix|Annexure|Annex|purchase\s+order|order\s+form))/i,
