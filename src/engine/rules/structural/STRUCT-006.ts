...
function documentText(ctx: Pick<RuleContext, "tree">): string {
  const parts: string[] = [];
...
/**
 * The Title-Case terms this document uses and never defines, after every
 * exemption below. The ONE owner of that list: STRUCT-006 reports it and the
 * definitions report's "undefined-but-used" bucket prints it. The bucket used
 * to read the extractor's raw list, so a clean will's definitions CSV named the
 * testator's husband, daughter and sister — and its "Independent Executor" —
 * as undefined terms in the same run whose findings exempted all four.
 */
export function undefinedTermCandidates(ctx: {
  tree: RuleContext["tree"];
  extracted: Pick<ExtractedData, "definitions" | "parties">;
}): ExtractedData["definitions"]["undefined_capitalized"] {
  ...
  if (INCORPORATES_DEFINITIONS.test(documentText(ctx)) || borrowsParentVocabulary(ctx)) return [];
  ...
  const title = (dropLegends([headingText, ...openingLines].filter((l) => l.length > 0))[0] ?? "")
    ...
    .toLowerCase();
  return ctx.extracted.definitions.undefined_capitalized.filter((e) => {
...
export const rule: Rule = {
  ...
  check(ctx: RuleContext): Finding | null {
    const candidates = undefinedTermCandidates(ctx);
