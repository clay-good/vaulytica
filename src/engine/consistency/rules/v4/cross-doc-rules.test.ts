...
describe("CROSS-PARTY-001", () => {
  it("fires when the same party appears as 'Acme Corp.' in one doc and 'Acme, Inc.' in another", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Agreement", "This Master Services Agreement is between Acme Corp., a Delaware corporation (\"Provider\"), and Globex Industries, Inc., a New York corporation (\"Customer\")."],
    );
    const baa = makeDoc("baa", "baa",
      ["Agreement", "This Business Associate Agreement is between Acme, Inc., a Delaware corporation (\"Business Associate\"), and Globex Industries, Inc., a New York corporation (\"Covered Entity\")."],
    );
    const run = await runConsistency({ rules: [CROSS_PARTY_001], documents: [msa, baa], dkb: STARTER_DKB });
  ...
  it("does not fire when both documents use the same party name", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Agreement", "This Agreement is between Acme Corp., a Delaware corporation, and Globex Industries, Inc."],
    );
    const baa = makeDoc("baa", "baa",
      ["Agreement", "This Agreement is between Acme Corp., a Delaware corporation, and Globex Industries, Inc."],
    );
...
describe("CROSS-JURIS-001", () => {
  it("fires on a 3-document bundle with two different governing laws", async () => {
    ...
    const run = await runConsistency({ rules: [CROSS_JURIS_001], documents: [msa, dpa, baa], dkb: STARTER_DKB });
  ...
  it("does not fire when a 3-document bundle agrees on one governing law", async () => {
    ...
    const baa = makeDoc("baa", "baa",
      ["Governing Law", "This BAA shall be governed by the laws of the State of Delaware."],
    );
    const run = await runConsistency({ rules: [CROSS_JURIS_001], documents: [msa, dpa, baa], dkb: STARTER_DKB });
    expect(run.findings).toHaveLength(0);
  });
  ...
  it("does not fire on a 2-document bundle (CC-005 covers that)", async () => {
    ...
    const run = await runConsistency({ rules: [CROSS_JURIS_001], documents: [msa, dpa], dkb: STARTER_DKB });
    expect(run.findings).toHaveLength(0);
  });
});
...
describe("CROSS-DEFTERM-001", () => {
  it("fires when 'Customer Data' is defined differently across docs", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Definitions", "\"Customer Data\" means data that Customer provides to Provider through the Services."],
    );
    const dpa = makeDoc("dpa", "dpa-controller-processor",
      ["Definitions", "\"Customer Data\" means any data processed by Provider on behalf of Customer, including derivative analytics."],
    );
    const run = await runConsistency({ rules: [CROSS_DEFTERM_001], documents: [msa, dpa], dkb: STARTER_DKB });
    expect(run.findings.length).toBeGreaterThanOrEqual(1);
  ...
  it("does not fire when definitions match", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Definitions", "\"Customer Data\" means data Customer provides to Provider."],
    );
    const dpa = makeDoc("dpa", "dpa-controller-processor",
      ["Definitions", "\"Customer Data\" means data Customer provides to Provider."],
    );
...
describe("CROSS-DATE-001", () => {
  it("fires when MSA dated 2026-01-01 references a BAA dated 2026-06-01", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Agreement", "This Master Services Agreement is effective as of January 1, 2026. The Business Associate Agreement is incorporated by reference."],
    );
    const baa = makeDoc("baa", "baa",
      ["Agreement", "This Business Associate Agreement is effective as of June 1, 2026."],
    );
    const run = await runConsistency({ rules: [CROSS_DATE_001], documents: [msa, baa], dkb: STARTER_DKB });
    expect(run.findings.length).toBeGreaterThanOrEqual(1);
  ...
  it("does not fire when the chronology is sane", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Agreement", "This Master Services Agreement is effective as of July 1, 2026. The Business Associate Agreement is incorporated by reference."],
    );
    const baa = makeDoc("baa", "baa",
      ["Agreement", "This Business Associate Agreement is effective as of June 1, 2026."],
    );
...
describe("CROSS-AMOUNT-001", () => {
  it("fires when aggregate liability caps differ across docs", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Limitation of Liability", "Provider's aggregate liability under this Agreement shall not exceed $1,000,000."],
    );
    const sow = makeDoc("sow", "sow",
      ["Limitation of Liability", "Provider's aggregate liability under this Statement of Work shall not exceed $50,000."],
    );
    const run = await runConsistency({ rules: [CROSS_AMOUNT_001], documents: [msa, sow], dkb: STARTER_DKB });
    expect(run.findings).toHaveLength(1);
  ...
  it("does not fire when caps match", async () => {
    ...
    const sow = makeDoc("sow", "sow",
      ["Limitation of Liability", "Provider's aggregate liability under this SOW shall not exceed $1,000,000."],
    );
    const run = await runConsistency({ rules: [CROSS_AMOUNT_001], documents: [msa, sow], dkb: STARTER_DKB });
    expect(run.findings).toHaveLength(0);
  });
...
describe("CROSS-MISSING-001", () => {
  it("fires when MSA references a Data Processing Agreement that isn't in the bundle", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Privacy", "Customer Data is processed under the Data Processing Agreement attached as Schedule B."],
    );
    // No DPA in the bundle.
    const baa = makeDoc("baa", "baa",
      ["Agreement", "Some BAA content."],
    );
    const run = await runConsistency({ rules: [CROSS_MISSING_001], documents: [msa, baa], dkb: STARTER_DKB });
    expect(run.findings.length).toBeGreaterThanOrEqual(1);
  ...
  it("does not fire when the referenced companion is present", async () => {
    ...
    const dpa = makeDoc("dpa", "dpa-controller-processor",
      ["Agreement", "This Data Processing Agreement governs Customer Data."],
    );
    const run = await runConsistency({ rules: [CROSS_MISSING_001], documents: [msa, dpa], dkb: STARTER_DKB });
    expect(run.findings).toHaveLength(0);
  });
});
...
describe("CROSS-PRECEDENCE-001", () => {
  it("fires when both MSA and SOW claim to control", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Order of Precedence", "In the event of any conflict between this Agreement and any Statement of Work, this Agreement shall control."],
    );
    const sow = makeDoc("sow", "sow",
      ["Order of Precedence", "In the event of any conflict between this Statement of Work and the Master Services Agreement, this Statement of Work shall control."],
    );
    const run = await runConsistency({ rules: [CROSS_PRECEDENCE_001], documents: [msa, sow], dkb: STARTER_DKB });
    expect(run.findings).toHaveLength(1);
  ...
  it("does not fire when only one document claims controlling status", async () => {
    ...
    const sow = makeDoc("sow", "sow",
      ["Scope", "The Services are described herein. The parties agree the MSA defines defined terms."],
    );
    const run = await runConsistency({ rules: [CROSS_PRECEDENCE_001], documents: [msa, sow], dkb: STARTER_DKB });
    expect(run.findings).toHaveLength(0);
  });
...
describe("V4_CROSS_RULES — determinism", () => {
  it("produces identical result_hash on repeated runs", async () => {
    const msa = makeDoc("msa", "msa-vendor-deep",
      ["Agreement", "This MSA is between Acme Corp., a Delaware corporation, and Globex Industries, Inc."],
